...
Hanbao 一个版本会发布四种产物——**PyPI** wheel、**Docker** 镜像、**桌面**应用
...
1. **建草稿 Release**
   ...
   - CLI：
     ...
     ```
   - tag 应与 `src/hanbao/__version__.py` 对应（`resolve` 会用 `packaging` 归一化校验，
...
| resolve 拒绝该 tag（版本不匹配） | 草稿 tag 与 `src/hanbao/__version__.py` 不一致 | 让 tag 与版本对齐（`packaging` 归一化，如 `v2.0.1-beta.1` ↔ `2.0.1b1`），再重跑。 |
...
`HANBAO_DASHSCOPE_API_KEY` secret——`dry_run` 只跳过 resolve 阶段的 fail-fast 检查，
不跳过验证本身。
