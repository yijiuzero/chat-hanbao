...
> 文档版本：v0.2 · 日期：2026-08-28 · 状态：阶段 0~6 全部完成（FPK 真机验证通过 2026-08-27）；阶段 7 上架飞牛应用中心进行中
...
| 阶段4 容器化 | 自建瘦身镜像（去 XFCE4/Chromium）+ compose 验证 | 本地 Docker 跑通（体积大幅缩小） | ✅ 已完成（1.23GB） |
| 阶段5 FPK 打包 | fnpack 结构 + wizard + 环境变量映射 | `.fpk` 产出 | ✅ 已完成（hanbao.fpk，native 形态离线镜像，2026-08-27 真机验证通过） |
| 阶段6 飞牛实测 | 应用中心手动安装，验证聊天/模型/持久化/认证 | 实测报告 | ✅ 已完成（2026-08-27 启用成功） |
| 阶段7 上架/发布 | 提交飞牛应用中心 | 上架或私有分发 | 🟡 进行中（下一步） |
