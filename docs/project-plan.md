...
- 模型：云 API 配置（安装向导填 Key，默认 DashScope / OpenAI）+ 可选本地小模型
...
| 阶段 | 内容 | 产出 |
| --- | --- | --- |
| 阶段0 准备 | 源码就位（本地路径）、建 git 仓库、锁定 v2.0.1 | 工程骨架 |
| 阶段1 品牌改造 | 改前端/CLI 别名/数据目录/遥测/文档 | 品牌层替换完成 |
| 阶段2 删减定制 | 按清单裁剪，跑通测试 | 裁剪后代码可运行 |
| 阶段3 容器化 | 自建瘦身镜像 + compose 验证 | 本地 Docker 跑通 |
| 阶段4 FPK 打包 | fnpack 结构 + wizard + 环境变量映射 | `.fpk` 产出 |
| 阶段5 飞牛实测 | 应用中心手动安装，验证聊天/模型/持久化/认证 | 实测报告 |
| 阶段6 上架/发布 | 提交飞牛应用中心 | 上架或私有分发 |
