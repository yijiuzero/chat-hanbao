...
> 文档版本：v0.1 · 日期：2026-08-07 · 状态：待评审
...
源码本地路径：`E:\浏览器下载\QwenPaw-2.0.1\QwenPaw-2.0.1`（用户已下载，未自行从网络拉取）。

| 维度 | 核实结论（来自 README.md / pyproject.toml / docker-compose.yml） |
| --- | --- |
| 版本 | v2.0.1，2026-07-24 发布；基于 AgentScope 2.0（`agentscope==2.0.4.post1`） |
| 技术栈 | Python 包 `qwenpaw`（`requires-python >=3.11,<3.14`）；后端 **uvicorn（ASGI）**；前端 React console（构建产物拷至 `src/qwenpaw/console/`） |
| 运行形态 | CLI `qwenpaw app`（另有 `copaw` 别名）、Docker（`agentscope/qwenpaw:latest`）、桌面 App（Tauri Beta）、TUI |
...
| 遥测 | `qwenpaw init` 时上报匿名使用数据（版本/安装方式/OS/Python/CPU 架构/GPU） |
...
**结论：可行，但有资源门槛。** 模型默认引导**云 API**（DashScope / OpenAI 等）；镜像需关注飞牛下载体积，建议自建镜像仓库或多阶段构建瘦身。
...
| 认证默认关闭 | 飞牛对外暴露风险 | FPK wizard 强制设管理员账号密码，开启 `QWENPAW_AUTH_*` |
...
> 锁定 QwenPaw v2.0.1 独立 fork → 最小必要品牌改造 → 按需求删减 → Docker 化（自建瘦身镜像）→ FPK 打包上架飞牛应用中心。模型以云 API 为主、本地小模型为辅，强制管理员认证。
...
1. **展示名**：`hanbao` 还是中文名（如"汉堡"）？
2. **保留范围**：是否保留 TUI / Coding Mode / 多渠道？MVP 建议只保留 Web Console 聊天。
3. **默认模型来源**：云 API（向导填 Key）还是引导本地小模型？建议云 API 优先。
4. **版本号起始**：建议 `1.0.0`（基于 QwenPaw 2.0.1）。
5. **镜像托管**：自建 registry 还是每次 `fnpack` 内嵌构建？
6. **包名策略**：MVP 先改"用户可见品牌层"（前端/CLI 别名/数据目录/图标/遥测/文档），Python 包名 `qwenpaw` 暂保留（避免大规模改 import 引入 bug）；后续版本再彻底改名。是否接受？
