# -*- coding: utf-8 -*-
"""
Hanbao Backups module end-to-end test cases.
...
class TestBackupPageDisplay:
    ...
    def test_backup_page_load_and_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            table = page.locator(".hanbao-table").first
            empty_state = page.locator(".hanbao-empty, [class*='empty']").first

            if table.is_visible(timeout=5000):
                # Has backup data: verify column headers
                headers = page.locator(".hanbao-table-thead th").all()
...
class TestCreateBackupModalAndCancel:
    ...
    def test_create_backup_modal_and_cancel(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            modal = page.locator(".hanbao-modal, .hanbao-drawer").first
            expect(modal).to_be_visible(timeout=5000)
            logger.info("Create-backup modal/drawer opened")

            # Verify modal title
            modal_title = modal.locator(
                '.hanbao-modal-title, .hanbao-drawer-title, h2, h3'
            ).first
            ...
            close_btn = modal.locator('.hanbao-modal-close, .hanbao-drawer-close').first
            ...
            # Verify modal closed
            modal_still = page.locator(".hanbao-modal, .hanbao-drawer").first
            if modal_still.is_visible(timeout=2000):
                logger.warning("Modal did not fully close, trying Escape key")
                page.keyboard.press("Escape")
                page.wait_for_timeout(500)

            expect(page.locator(".hanbao-modal, .hanbao-drawer").first).not_to_be_visible(timeout=5000)
...
class TestCreateFullBackup:
    ...
    def test_create_full_backup(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            initial_rows = page.locator(".hanbao-table-tbody tr").all()
            ...
            name_input = modal.locator(
                'input[placeholder*="name"], input[placeholder*="名称"], input.hanbao-input'
            ).first
            ...
            confirm_btn = modal.locator(
                'button.hanbao-btn-primary, button:has-text("OK"), '
            ...
            progress = page.locator('.hanbao-progress, [class*="progress"]').first
            ...
            if not creation_confirmed:
                ...
                modal_gone = not page.locator(".hanbao-modal, .hanbao-drawer").first.is_visible(timeout=2000)
                if modal_gone:
            ...
            final_rows = page.locator(".hanbao-table-tbody tr").all()
            ...
            rows = page.locator(".hanbao-table-tbody tr").all()
            if len(rows) > 0:
                ...
                if not restore_btn.is_visible(timeout=3000):
                    more_btn = target_row.locator(
                        'button[class*="more"], .hanbao-dropdown-trigger, '
                    ...
                    if more_btn.is_visible(timeout=3000):
                        ...
                        restore_btn = page.locator(
                            '.hanbao-dropdown-menu [class*="restore"], '
                            '.hanbao-dropdown-menu :has-text("Restore"), '
                            '.hanbao-dropdown-menu :has-text("恢复")'
                        ).first
                ...
                if restore_btn.is_visible(timeout=3000):
                    ...
                    restore_modal = page.locator(".hanbao-modal, .hanbao-drawer").first
                    restore_confirm = page.locator('.hanbao-popconfirm, .hanbao-popover').first
                    ...
                    if restore_modal.is_visible(timeout=5000):
                        ...
                        pop_ok = page.locator(
                            '.hanbao-popconfirm button.hanbao-btn-primary, '
                            '.hanbao-popconfirm button:has-text("OK"), '
                            '.hanbao-popconfirm button:has-text("确定"), '
                            '.hanbao-popconfirm button:has-text("Yes"), '
                            '.hanbao-popconfirm button:has-text("是")'
                        ).first
                    ...
                    restore_progress = page.locator('.hanbao-progress, [class*="progress"]').first
                    ...
                    if restore_success_msg.is_visible(timeout=30000):
                        ...
                        modal_gone = not page.locator(".hanbao-modal, .hanbao-drawer").first.is_visible(timeout=3000)
                        if modal_gone:
            ...
            if backup_created:
                try:
                    ...
                    leftover_modal = page.locator(".hanbao-modal, .hanbao-drawer").first
                    ...
                    if len(rows) > 0:
                        ...
                        if not delete_btn.is_visible(timeout=3000):
                            ...
                            if more_btn.is_visible(timeout=3000):
                                ...
                                delete_btn = page.locator(
                                    '.hanbao-dropdown-menu :has-text("Delete"), '
                                    '.hanbao-dropdown-menu :has-text("删除")'
                                ).first
                        ...
                        if delete_btn.is_visible(timeout=3000):
                            ...
                            confirm_delete = page.locator(
                                ...
                                '.hanbao-modal button:has-text("OK"), '
                                '.hanbao-modal button:has-text("确定")'
                            ).first
    ...
    def test_backup_search_and_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            search_input = page.locator(
                '.hanbao-input-search input, input[placeholder*="search"], '
            ...
            filtered_rows = page.locator(".hanbao-table-tbody tr").all()
            ...
            restored_rows = page.locator(".hanbao-table-tbody tr").all()
...
class TestBackupRestoreModal:
    ...
    def test_backup_restore_modal(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            modal = page.locator(".hanbao-modal, .hanbao-drawer").first
            confirm_dialog = page.locator('.hanbao-popconfirm, .hanbao-popover').first
    ...
    def test_backup_delete_and_cancel(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            cancel_btn = page.locator(
                '.hanbao-popconfirm button:has-text("Cancel"), '
                '.hanbao-popconfirm button:has-text("取消"), '
                '.hanbao-modal button:has-text("Cancel"), '
                '.hanbao-modal button:has-text("取消"), '
            ...
            page.wait_for_timeout(1000)
            after_rows = page.locator(".hanbao-table-tbody tr").all()
            ...
            if not export_btn.is_visible(timeout=3000):
                ...
                if more_btn.is_visible(timeout=3000):
                    ...
                    export_btn = page.locator(
                        '.hanbao-dropdown-menu :has-text("Export"), '
                        '.hanbao-dropdown-menu :has-text("导出"), '
                        '.hanbao-dropdown-menu :has-text("Download"), '
                        '.hanbao-dropdown-menu :has-text("下载")'
                    ).first
    ...
    def test_create_partial_backup_options(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            has_partial_content = (
                ...
                or modal.locator('input[type="checkbox"], .hanbao-checkbox, .hanbao-select, .hanbao-switch').first.is_visible(timeout=3000)
            ...
            checkboxes = modal.locator('.hanbao-checkbox, .hanbao-switch').all()
...
class TestBackupListRefreshAndEmpty:
    ...
    def test_backup_list_refresh_and_empty(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            initial_rows = page.locator(".hanbao-table-tbody tr").all()
            initial_count = len(initial_rows)
            has_empty = page.locator(".hanbao-empty, [class*='empty']").first.is_visible(timeout=2000)
            ...
            refreshed_rows = page.locator(".hanbao-table-tbody tr").all()
            refreshed_count = len(refreshed_rows)
            refreshed_empty = page.locator(".hanbao-empty, [class*='empty']").first.is_visible(timeout=2000)
            ...
            if refreshed_count == 0:
                ...
                empty_el = page.locator(".hanbao-empty, [class*='empty']").first
