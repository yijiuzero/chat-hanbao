...
def pytest_sessionstart(session):
    """Hard guard against running e2e against a real Hanbao instance.

    Triggers ``config.working_dir`` validation before any fixture
    chain runs. Fails the entire session in <1s if
    ``HANBAO_WORKING_DIR`` is unset or points inside the user's home
...
_DEFAULT_PROVIDER = os.getenv("HANBAO_MODEL_PROVIDER", "dashscope")
_DEFAULT_MODEL = os.getenv("HANBAO_DEFAULT_MODEL", "qwen3.6-plus")
...
def setup_default_model(api_context):
    """Configure provider API key and set a global default model when HANBAO_DASHSCOPE_API_KEY is set."""
    model_key = os.getenv("HANBAO_DASHSCOPE_API_KEY")
    if not model_key:
        logger.info("HANBAO_DASHSCOPE_API_KEY not set, skipping model setup")
        yield
        return
