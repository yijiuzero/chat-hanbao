# -*- coding: utf-8 -*-
"""
Hanbao Token Usage module P0 end-to-end tests
...
class TestTokenUsageDisplay:
    ...
    def test_token_usage_overview(self, page: Page, request: pytest.FixtureRequest):
        ...
        page_title = page.locator('h1:has-text("Token Usage"), h1:has-text("Token"), .hanbao-page-header:has-text("Token")').first
        ...
        overview_cards = page.locator(
            '.hanbao-card, .ant-card, [class*=overviewCard], '
            '[class*=statCard], .hanbao-statistic, .ant-statistic'
        ).all()
        ...
        table_area = page.locator('.hanbao-table, .ant-table, table, [class*=dataTable]').first
        empty_state = page.locator('.hanbao-empty, .ant-empty, [class*=empty], [class*=Empty]').first
...
class TestTokenUsageByModel:
    ...
    def test_token_usage_by_model(self, page: Page, request: pytest.FixtureRequest):
        ...
        tables = page.locator('.hanbao-table, table').all()
        ...
        if len(tables) > 0:
            ...
            headers = first_table.locator('th, .hanbao-table-thead th').all()
            ...
            data_rows = first_table.locator('tbody tr, .hanbao-table-row').all()
            empty_state = first_table.locator('.hanbao-empty, :text("暂无"), :text("No data")').first
...
class TestTokenUsageByDate:
    ...
    def test_token_usage_by_date(self, page: Page, request: pytest.FixtureRequest):
        ...
        if len(tables) >= 2:
            ...
            headers = date_table.locator('th, .hanbao-table-thead th').all()
            header_texts = [h.inner_text().strip() for h in headers if h.is_visible()]
            ...
            data_rows = date_table.locator('tbody tr, .hanbao-table-row').all()
            ...
            tabs = page.locator('.hanbao-tabs-tab, .hanbao-segmented-item').all()
            if len(tabs) > 0:
...
class TestTokenUsageDateFilter:
    ...
    def test_token_usage_date_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        range_picker = page.locator(
            '.hanbao-picker-range, .ant-picker-range, '
        ...
        if range_picker.count() > 0:
            ...
            date_panel = page.locator(
                '.hanbao-picker-dropdown, .ant-picker-dropdown, '
                '.hanbao-picker-panel, .ant-picker-panel'
            ).first
            if date_panel.count() > 0:
                ...
                today_cell = date_panel.locator(
                    '.hanbao-picker-cell-today, .ant-picker-cell-today, '
                    'td.hanbao-picker-cell-in-view'
                ).first
                if today_cell.count() > 0 and today_cell.is_visible(timeout=2000):
                    ...
                    if is_range:
                        ...
                        end_cells = date_panel.locator('td.hanbao-picker-cell-in-view, td.ant-picker-cell-in-view').all()
                        if len(end_cells) > 1:
                            end_cells[-1].click()
                            page.wait_for_timeout(500)
...
class TestTokenUsageEmptyState:
    ...
    def test_token_usage_empty_state(self, page: Page, request: pytest.FixtureRequest):
        ...
        loading = page.locator('.hanbao-spin, .ant-spin, [class*="loading"]').first
        ...
        empty_state = page.locator('.hanbao-empty, :text("暂无"), :text("No data"), :text("Empty")').first
        tables = page.locator('.hanbao-table, table').all()
        cards = page.locator('.hanbao-card').all()
