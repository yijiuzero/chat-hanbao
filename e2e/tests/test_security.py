# -*- coding: utf-8 -*-
"""
Hanbao Security module P0-level end-to-end test cases.
...
class TestSecurityToolGuardAndTabSwitch:
    ...
    def test_tool_guard_toggle_and_tab_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_tab = page.locator('[data-node-key="toolGuard"] .hanbao-tabs-tab-btn').first
        file_guard_tab = page.locator('[data-node-key="fileGuard"] .hanbao-tabs-tab-btn').first
        ...
        if not active_panel.is_visible():
            ...
            active_panel = page.locator('.hanbao-tabs-tabpane-active').first
        ...
        tool_guard_switch = active_panel.locator('button.hanbao-switch[role="switch"]').first
        ...
        protected_tools_select = active_panel.locator('.hanbao-select').first
        ...
        file_guard_panel = page.locator('.hanbao-tabs-tabpane-active').first
        ...
        file_guard_switch = file_guard_panel.locator('button.hanbao-switch[role="switch"]').first
...
class TestSecurityFileGuardPathAndToolSelect:
    ...
    def test_file_guard_path_add_and_tool_select(self, page: Page, request: pytest.FixtureRequest):
        ...
        add_button = file_guard_panel.locator('button.hanbao-btn-primary').first
        ...
        tool_guard_panel = page.locator('.hanbao-tabs-tabpane-active').first
        ...
        protected_tools_select = tool_guard_panel.locator('.hanbao-select').first
        expect(protected_tools_select).to_be_visible(timeout=5000)
        # Click the selector inside the Select to trigger the dropdown
        select_selector = protected_tools_select.locator('.hanbao-select-selector').first
        ...
        dropdown = page.locator('.hanbao-select-dropdown:visible').first
...
class TestSecurityConfigSaveAndPersist:
    ...
    def test_security_config_save_and_persist(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_switch = tool_guard_panel.locator('button.hanbao-switch[role="switch"]').first
        expect(tool_guard_switch).to_be_visible(timeout=5000)
        ...
        try:
            ...
            save_btn = page.locator('button.hanbao-btn-primary:has-text("保存"), button:has-text("保 存")').first
            if not save_btn.is_visible():
                save_btn = page.locator('div[class*="footer"] button.hanbao-btn-primary').first
            ...
            success_msg = page.locator('.hanbao-message-success, .hanbao-message-notice-content:has-text("保存")').first
            ...
            tool_guard_tab_refreshed = page.locator('[data-node-key="toolGuard"] .hanbao-tabs-tab-btn').first
            expect(tool_guard_tab_refreshed).to_be_visible(timeout=5000)
            tool_guard_tab_refreshed.click()
            page.wait_for_timeout(1500)

            tool_guard_panel_refreshed = page.locator('.hanbao-tabs-tabpane-active').first
            expect(tool_guard_panel_refreshed).to_be_visible(timeout=5000)

            tool_guard_switch_refreshed = tool_guard_panel_refreshed.locator('button.hanbao-switch[role="switch"]').first
            ...
            try:
                tool_guard_switch_refreshed = page.locator('.hanbao-tabs-tabpane-active').first.locator('button.hanbao-switch[role="switch"]').first
                if tool_guard_switch_refreshed.is_visible():
                    ...
                    if current_state != initial_checked:
                        ...
                        # Save again
                        save_btn_refreshed = page.locator('button.hanbao-btn-primary:has-text("保存"), button:has-text("保 存")').first
                        if not save_btn_refreshed.is_visible():
                            save_btn_refreshed = page.locator('div[class*="footer"] button.hanbao-btn-primary').first
        ...
        skill_scanner_tab = page.locator('[data-node-key="skillScanner"] .hanbao-tabs-tab-btn').first
        ...
        if skill_scanner_tab.is_visible():
            ...
            skill_scanner_panel = page.locator('.hanbao-tabs-tabpane-active').first
...
class TestSecurityRuleCrud:
    ...
    def test_security_rule_crud(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            modal = page.locator('.hanbao-modal').first
            ...
            confirm_btn = modal.locator('button.hanbao-btn-primary').first
            ...
            severity_tag = rule_row.locator('.hanbao-tag:has-text("HIGH")').first
            if severity_tag.count() == 0:
                severity_tag = rule_row.locator('.hanbao-tag').first
            ...
            enable_switch = rule_row.locator('button.hanbao-switch[role="switch"]').last
            ...
            edit_modal = page.locator('.hanbao-modal').first
            ...
            if rule_id:
                try:
                    ...
                    if cleanup_rule_row.count() > 0:
                        ...
                        delete_btn.click()
                        page.wait_for_timeout(1500)
                        confirm_delete_btn = page.locator('.hanbao-modal-confirm button.hanbao-btn-primary, .hanbao-modal button:has-text("确认"), .hanbao-modal button:has-text("Delete")').first
...
class TestSkillScannerModeSwitch:
    ...
    def test_skill_scanner_mode_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        mode_select = skill_scanner_panel.locator('.hanbao-select').first
        ...
        current_mode_selector = mode_select.locator('.hanbao-select-selector').first
        ...
        try:
            ...
            block_option = page.locator('.hanbao-select-item:has-text("block"), .hanbao-select-item:has-text("Block")').first
            ...
            warn_option = page.locator('.hanbao-select-item:has-text("warn"), .hanbao-select-item:has-text("Warn")').first
            ...
            off_option = page.locator('.hanbao-select-item:has-text("off"), .hanbao-select-item:has-text("Off")').first
            ...
            timeout_input = skill_scanner_panel.locator('input[type="number"], .hanbao-input-number input').first
            ...
            scan_alerts_tab = skill_scanner_panel.locator('[data-node-key="scanAlerts"] .hanbao-tabs-tab-btn, .hanbao-tabs-tab-btn:has-text("扫描警报"), .hanbao-tabs-tab-btn:has-text("Scan Alerts")').first
            ...
            whitelist_tab = skill_scanner_panel.locator('[data-node-key="whitelist"] .hanbao-tabs-tab-btn, .hanbao-tabs-tab-btn:has-text("白名单"), .hanbao-tabs-tab-btn:has-text("Whitelist")').first
            ...
            if original_mode_text and current_mode_selector:
                try:
                    current_mode_selector.click()
                    page.wait_for_timeout(1000)
                    restore_option = page.locator(f'.hanbao-select-item:has-text("{original_mode_text}")').first
...
class TestDeniedToolsConfig:
    ...
    def test_denied_tools_config(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_tab = page.locator(
            '[data-node-key="toolGuard"] .hanbao-tabs-tab-btn, '
            '.hanbao-tabs-tab-btn:has-text("Tool Guard"), '
            '.hanbao-tabs-tab-btn:has-text("工具防护")'
        ).first
        ...
        active_panel = page.locator('.hanbao-tabs-tabpane-active').first
        guard_switch = active_panel.locator('.hanbao-switch').first
        ...
        denied_tools_select = page.locator(
            '#denied_tools, '
            '.hanbao-select:near(:text("Denied"), 200), '
            '.hanbao-select:near(:text("拒绝"), 200)'
        ).first
        ...
        if denied_tools_select.count() > 0:
            ...
            if len(options) > 0:
                ...
                # Verify selection
                selected_tags = page.locator('.hanbao-select-selection-item').all()
            ...
            form_items = active_panel.locator('.hanbao-form-item').all()
...
class TestRulePreview:
    ...
    def test_rule_preview(self, page: Page, request: pytest.FixtureRequest):
        ...
        active_panel = page.locator('.hanbao-tabs-tabpane-active').first
        rule_table = active_panel.locator('table, .hanbao-table').first
        ...
        if rule_table.count() > 0:
            ...
            # Count rule rows
            rule_rows = active_panel.locator('table tbody tr, .hanbao-table-row').all()
            ...
            if len(preview_btns) > 0:
                ...
                preview_modal = page.locator('.hanbao-modal').first
                if preview_modal.count() > 0:
                    ...
                    # Close the modal
                    close_btn = preview_modal.locator('.hanbao-modal-close, button:has-text("Close"), button:has-text("关闭"), button:has-text("OK")').first
...
class TestSecurityBatchRuleToggle:
    ...
    def test_security_batch_rule_toggle(self, page: Page, request: pytest.FixtureRequest):
        ...
        log_test_step("Find the rules table")
        active_panel = page.locator('.hanbao-tabs-tabpane-active').first
        rule_switches = active_panel.locator('.hanbao-switch').all()
