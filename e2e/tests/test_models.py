# -*- coding: utf-8 -*-
"""
Hanbao Local Models module P0 end-to-end test cases.
...
class TestModelListDisplay:
    ...
    def test_model_list_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        server_status = page.locator('[class*="serverStatus"], .hanbao-card:has-text("llama.cpp"), .hanbao-card:has-text("Server")').first
        ...
        provider_tiles = page.locator(
            ...
            '[class*="modelList"], .hanbao-list, .hanbao-card'
        ).all()
        ...
        provider_cards = page.locator('[class*="providerCard"], .hanbao-card').all()
        ...
        data_items = page.locator('[class*="modelItem"], .hanbao-list-item, .hanbao-table-row').all()
...
class TestModelServe:
    ...
    def test_model_serve_flow(self, page: Page, request: pytest.FixtureRequest):
        ...
        model_items = page.locator('[class*=modelItem], .hanbao-list-item, .hanbao-card').all()
        ...
        serve_btns = page.locator('button:has-text("启动"), button:has-text("Serve"), button:has-text("服务"), .hanbao-btn:has-text("启动")').or_(page.get_by_text("启动")).or_(page.get_by_text("Serve")).or_(page.get_by_text("服务")).all()
        ...
        if len(serve_btns) > 0:
            ...
            response_indicators = page.locator(
                '.hanbao-modal, .hanbao-drawer, '
                '.hanbao-message, .hanbao-notification, '
        ...
        status_display = page.locator('[class*="status"], [class*="serving"], .hanbao-tag, .hanbao-badge').first
...
class TestModelManagement:
    ...
    def test_model_management_operations(self, page: Page, request: pytest.FixtureRequest):
        ...
        more_btns = page.locator('button:has-text("⋮"), button:has-text("⋯"), .hanbao-btn-icon:has(.spark-icon-spark-more-line)').all()
        ...
        if len(more_btns) > 0:
            ...
            delete_option = page.locator('.hanbao-dropdown-menu-item:has-text("删除"), .hanbao-dropdown-menu-item:has-text("Delete")').or_(page.get_by_text("删除")).or_(page.get_by_text("Delete")).first
        ...
        running_status = page.locator('[class*=running], .hanbao-tag:has-text("运行中")').or_(page.get_by_text("运行中")).or_(page.get_by_text("Running")).first
...
class TestCustomProviderCreateAndDelete:
    ...
    def test_custom_provider_create_and_delete(self, page: Page):
        ...
        ok_button = modal.locator("button.hanbao-btn-primary").first
        ...
        delete_btn = provider_card.locator("button.hanbao-btn-dangerous, button[class*='dangerous']").first
        ...
        confirm_modal = page.locator(".hanbao-modal-confirm, .hanbao-modal, .hanbao-popconfirm").first
        if confirm_modal.count() > 0:
            ...
            if confirm_btn.count() > 0:
                ...
                footer_btn = confirm_modal.locator(".hanbao-modal-confirm-btns button, .hanbao-modal-footer button").last
...
class TestProviderConfigAndConnection:
    ...
    def test_provider_config_and_connection_test(self, page: Page):
        ...
        try:
            ...
            log_test_step("Submit the create form")
            create_button = modal.locator("button.hanbao-btn-primary").first
            ...
            try:
                ...
                provider_card = page.locator(
                    f".hanbao-card:has-text('{provider_id}'), "
                ...
                if provider_card.count() > 0:
                    ...
                    if delete_btn.count() > 0:
                        ...
                        confirm_modal = page.locator(".hanbao-modal-confirm, .hanbao-modal, .hanbao-popconfirm").first
                        if confirm_modal.count() > 0:
                            confirm_btn = confirm_modal.locator("button.hanbao-btn-primary, button:has-text('确 定'), button:has-text('OK')").first
                            if confirm_btn.count() > 0:
                                confirm_btn.click()
                                page.wait_for_timeout(2000)
...
class TestProviderSearchFilter:
    ...
    def test_provider_search_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        search_input = page.locator('input[type="search"], input[placeholder*="search"], input[placeholder*="Search"], input[placeholder*="搜索"], .hanbao-input-search input').first
        ...
        # Search box is a hanbao-select component (readonly input); need to click the parent container to open dropdown
        ...
        if is_readonly:
            ...
            select_container = page.locator('.hanbao-select').first
        ...
        if is_readonly:
            ...
            clear_btn = page.locator('.hanbao-select-clear').first
    ...
    def test_model_activation(self, page: Page, request: pytest.FixtureRequest):
        ...
        if models_btn.count() > 0:
            ...
            if modal.count() > 0:
                ...
                # Close modal
                close_btn = modal.locator('.hanbao-modal-close, button:has-text("Cancel"), button:has-text("取消")').first
                if close_btn.count() > 0:
                    close_btn.click()
                    page.wait_for_timeout(1000)
            else:
                logger.info("No model management modal, may be a Drawer instead")
                drawer = page.locator('.hanbao-drawer').first
                if drawer.count() > 0:
            ...
            has_response = page.locator('.hanbao-modal, .hanbao-drawer').first.count() > 0
...
class TestOpenRouterFilter:
    ...
    def test_openrouter_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        config_modal = page.locator(
            '.hanbao-modal:has-text("OpenRouter"), '
            '.ant-modal:has-text("OpenRouter"), '
            '.hanbao-modal:has-text("Base URL"), '
...
class TestModelJsonEditor:
    ...
    def test_model_json_editor(self, page: Page, request: pytest.FixtureRequest):
        ...
        provider_cards = page.locator('.hanbao-card').all()
        ...
        page.wait_for_timeout(500)
        modal_or_drawer = page.locator('.hanbao-modal, .ant-modal, .hanbao-drawer, .ant-drawer').first
