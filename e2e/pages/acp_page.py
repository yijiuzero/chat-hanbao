# -*- coding: utf-8 -*-
"""
Hanbao ACP (Agent Communication Protocol) page object.
...
class ACPPage(BasePage):
    ...
    # Filter tabs
    FILTER_TABS = '.hanbao-tabs, .hanbao-segmented, [class*="filterTabs"]'
    TAB_ALL = '[class*="tab"]:has-text("All"), [class*="tab"]:has-text("全部"), .hanbao-segmented-item:has-text("All")'
    TAB_BUILTIN = '[class*="tab"]:has-text("Builtin"), [class*="tab"]:has-text("内置"), .hanbao-segmented-item:has-text("Builtin")'
    TAB_CUSTOM = '[class*="tab"]:has-text("Custom"), [class*="tab"]:has-text("自定义"), .hanbao-segmented-item:has-text("Custom")'

    # Create button
    CREATE_BUTTON = 'button:has-text("Create"), button:has-text("创建"), button:has-text("Add"), button:has-text("添加")'

    # ACP card list
    ACP_CARD = '[class*="acpCard"], [class*="ACPCard"], .hanbao-card'
    ACP_CARD_TITLE = '[class*="agentKey"], [class*="title"], .hanbao-card-meta-title'
    ACP_CARD_TAG = '.hanbao-tag'
    ACP_CARD_SWITCH = '.hanbao-switch'

    # ACP drawer (create/edit)
    DRAWER = '.hanbao-drawer'
    DRAWER_TITLE = '.hanbao-drawer-title'
    DRAWER_CLOSE = '.hanbao-drawer-close'
    ...
    FORM_ENABLED_SWITCH = '[class*="enabled"] .hanbao-switch, #enabled'
    FORM_TRUSTED_SWITCH = '[class*="trusted"] .hanbao-switch, #trusted'
    FORM_TOOL_PARSE_MODE = '.hanbao-select, select[id*="tool_parse_mode"]'
    FORM_BUFFER_LIMIT = 'input[id*="buffer"], input[name*="buffer"], input[type="number"]'

    # Drawer action buttons
    SAVE_BUTTON = '.hanbao-drawer button:has-text("Save"), .hanbao-drawer button:has-text("保存"), .hanbao-drawer button.hanbao-btn-primary'
    CANCEL_BUTTON = '.hanbao-drawer button:has-text("Cancel"), .hanbao-drawer button:has-text("取消")'
    DELETE_BUTTON_DRAWER = '.hanbao-drawer button:has-text("Delete"), .hanbao-drawer button:has-text("删除")'
    DOC_LINK = '.hanbao-drawer a[href*="doc"], .hanbao-drawer a[href*="integration"]'

    # Confirmation popups
    POPCONFIRM = '.hanbao-popconfirm, .hanbao-modal-confirm'
    POPCONFIRM_OK = '.hanbao-popconfirm button:has-text("OK"), .hanbao-popconfirm button:has-text("确定"), .hanbao-popconfirm .hanbao-btn-primary'
    POPCONFIRM_CANCEL = '.hanbao-popconfirm button:has-text("Cancel"), .hanbao-popconfirm button:has-text("取消")'
