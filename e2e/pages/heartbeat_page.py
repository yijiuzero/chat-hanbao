...
class HeartbeatPage(BasePage):
    ...
    PAGE_LOAD_INDICATOR = '.ant-switch, .hanbao-switch, input'

    # Configuration card
    CONFIG_CARD = ".ant-card, .hanbao-card, [class*=card]"
    CONFIG_FORM = ".ant-form, .hanbao-form"

    # Enabled switch (match id="enabled" exactly to avoid the "active hours" switch)
    ENABLED_SWITCH = '#enabled'
    ENABLED_LABEL = '.ant-form-item:has-text("Enable"), .ant-form-item:has-text("启用"), .hanbao-form-item:has-text("启用"), .hanbao-form-item:has-text("开启")'
    ...
    INTERVAL_UNIT_SELECT = '.hanbao-select:has(#everyUnit), .ant-select:has(#everyUnit), .ant-select:has-text("seconds"), .ant-select:has-text("minutes"), .ant-select:has-text("hours"), .hanbao-select:has-text("秒"), .hanbao-select:has-text("分钟"), .hanbao-select:has-text("小时")'

    # Scheduled time
    TIME_PICKER = '.ant-picker-input > input, .hanbao-picker-input > input'
    TIME_PICKER_PANEL = '.ant-picker-panel, .hanbao-picker-panel'

    # Skill configuration
    SKILL_SELECT = '.ant-select[data-placeholder*="Skill" i], .ant-select:has-text("skill"), .hanbao-select[data-placeholder*="技能" i], .hanbao-select:has-text("技能")'
    ...
    # Status indicator
    STATUS_INDICATOR = '.ant-badge-status, .hanbao-badge-status, .status-indicator'
    ...
    def is_heartbeat_enabled(self) -> bool:
        ...
        if switch.count() > 0:
            return switch.evaluate("el => el.classList.contains('ant-switch-checked') || el.classList.contains('hanbao-switch-checked') || el.getAttribute('aria-checked') === 'true'")
        return False
    ...
    def get_interval(self) -> Dict[str, Any]:
        ...
        if unit_select.count() > 0:
            ...
            selection_item = unit_select.first.locator('.hanbao-select-selection-item, .ant-select-selection-item')
            ...
            if unit_select.count() > 0:
                ...
                for alias in aliases:
                    option = self.page.locator(
                        f'.hanbao-select-item-option:has-text("{alias}"), '
                        f'.ant-select-item-option:has-text("{alias}"), '
                        f'.hanbao-select-item:has-text("{alias}"), '
    ...
    def assert_config_saved(self) -> "HeartbeatPage":
        ...
        error_msg = self.page.locator('.ant-message-error, .hanbao-message-error')
