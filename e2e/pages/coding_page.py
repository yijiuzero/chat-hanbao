# -*- coding: utf-8 -*-
"""
Hanbao Coding Mode page object.
...
- First-time activation triggers an "Experimental Feature" confirmation
  modal gated by ``localStorage["hanbao-coding-mode-confirmed"]``. To
...
class CodingPage(BasePage):
    """Page object for Hanbao Coding Mode."""
    ...
    # forced to ``en-US`` in fixtures, but the Hanbao frontend keeps a
    ...
    LS_KEY_CONFIRMED = "hanbao-coding-mode-confirmed"
    ...
    # via ``hanbao-last-used-agent`` (localStorage) and the persisted
    # zustand state under ``hanbao-agent-storage`` (both local and
    ...
    def _install_default_agent_init_script(self) -> None:
        ...
        rewrites both the legacy ``hanbao-last-used-agent`` key and the
        persisted ``hanbao-agent-storage`` blob in localStorage and
        ...
        script = (
            ...
            "      version: 0"
            "    });"
            "    try { localStorage.setItem('hanbao-last-used-agent', a); } catch (e) {}"
            "    try { localStorage.setItem('hanbao-agent-storage', blob); } catch (e) {}"
            "    try { sessionStorage.setItem('hanbao-agent-storage', blob); } catch (e) {}"
            "  } catch (e) {}"
            "})();"
        )
        try:
