...
Hanbao 提供了插件系统，允许用户扩展 Hanbao 的功能。
...
- **前端扩展插件**：在浏览器中运行的 JS 插件，共享宿主的 React / Ant Design 运行时，通过声明式 `window.Hanbao.*` API 扩展界面——注册侧边栏菜单、页面路由、UI 插槽、聊天定制等，无需修改宿主代码
...
| `hanbao_version` | `object`        | 否   | Hanbao 版本约束（推荐）。包含 `min`（包含）和 `max`（不包含，可选）两个子字段，语义为 `>=min, <max`。省略 `max` 时默认取 `{major}.{minor+1}.0`。 |
| `min_version`     | `string`        | 否   | **遗留字段。** 需要的最低 Hanbao 版本。当 `hanbao_version` 存在时被忽略，仅为兼容第三方旧插件保留。                                             |
| `max_version`     | `string`        | 否   | **遗留字段。** 不兼容的第一个 Hanbao 版本（不包含）。配合 `min_version` 使用；省略时从 `min_version` 推导。                                      |
...
前端插件是运行在浏览器端的 JavaScript 扩展。与后端插件通过 Python `PluginApi` 注册能力不同，前端插件通过全局 `window.Hanbao.*` API 声明式地扩展 Console 界面。

**加载生命周期：**

1. Console 启动，在 `window.Hanbao` 上挂载 Host SDK（React、antd 等共享依赖）和注册 API（menu、route、slot、chat 等命名空间）
2. Console 请求 `/frontend_plugin` 获取已启用的前端插件列表
3. 逐一下载各插件的 JS bundle，通过 Blob URL 动态导入执行
4. 插件代码执行，调用 `window.Hanbao.*` 注册菜单、路由、聊天定制等 UI 扩展
...
├── plugin.json      # 插件清单（必需）
├── src/
│   └── index.tsx    # 入口点，调用 window.Hanbao.* API
├── package.json     # 依赖声明
...
插件入口文件在加载时执行，通过 `window.Hanbao.*` API 注册扩展：

```tsx
const { React, antd } = window.Hanbao.host;
const pluginId = "my-plugin";

// 调用 window.Hanbao.* API 注册菜单、路由、聊天定制等
// 详见下方「前端扩展 API」
```
...
可将 `console/src/plugins/types/hanbao.d.ts` 复制到插件项目中作为 `hanbao-host.d.ts`，获得完整的类型提示。

## 前端扩展 API

前端插件通过 `window.Hanbao.*` API 扩展 Console 界面，无需修改宿主代码。所有注册方法第一个参数是 `pluginId`，每个注册返回 `{ dispose() }` 对象用于撤销。
...
`/api` 加上你指定的前缀下，与 Hanbao 核心 API 使用同一个 FastAPI 应用，因此
...
启动 Hanbao 后，可在终端用 `curl` 测试（端口请按你本地实际为准，例如 `8088`）：
...
本示例展示如何注册一个 `on_acting` middleware，当设置环境变量 `HANBAO_TRACE` 时记录每次 tool call 的名称、参数和执行耗时。
...
- **条件激活**：工厂函数检测环境变量 `HANBAO_TRACE`，仅设置时启用
...
Channel 插件可以为 Hanbao 添加新的消息平台。注册后的频道会在控制台 UI 中与内置
...
handler 必须继承 `hanbao.runtime.commands.control.base.BaseControlCommandHandler`，并实现 `command_name`、`help_text` 和 `async handle(self, ctx, args)` 方法。
...
一个为 Hanbao agents 添加 OpenAI GPT Image 2 图片生成能力的工具插件。
