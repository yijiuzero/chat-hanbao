# Hanbao Desktop Application Guide
...
This guide explains how to install and use the Hanbao Desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `Hanbao-Tauri-<version>-Windows-setup.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\Hanbao Desktop`
...
#### **Hanbao Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "Hanbao Desktop" icon on desktop or Start Menu
- **Technical Note**: Native Tauri desktop app; runs the Python backend as a background sidecar

#### **Hanbao Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click the "Hanbao Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "Hanbao Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**

A: Go to Windows Settings → Apps → Installed apps → Find "Hanbao Desktop" → Uninstall
...
1. **Download the archive**
   Download `Hanbao-Tauri-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `Hanbao Desktop.app`

3. **Move to Applications folder (Optional)**
   Drag `Hanbao Desktop.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

Hanbao is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `Hanbao Desktop.app`
...
2. Scroll down to find a message like:
   _"'Hanbao' was blocked from use because it cannot verify the developer"_
...
xattr -cr "/Applications/Hanbao Desktop.app"
```
...
- Double-click `Hanbao Desktop.app` to launch
- The app runs in the background and opens its window
- App logs are written to: `~/Library/Logs/io.agentscope.hanbao.desktop/hanbao-desktop.log`
- Backend sidecar logs live in the working directory: `~/.hanbao/desktop.log`
...
HANBAO_DESKTOP_DEBUG=1 "/Applications/Hanbao Desktop.app/Contents/MacOS/hanbao-desktop"
```
...
- ✅ `HANBAO_DESKTOP_DEBUG=1` raises the desktop log level to debug for more detailed information

**View log files:**

```bash
# Tail the app log
tail -f ~/Library/Logs/io.agentscope.hanbao.desktop/hanbao-desktop.log

# Tail the backend sidecar log
tail -f ~/.hanbao/desktop.log
```
...
1. Check the `~/.hanbao/desktop.log` file for errors
...
A: Drag `Hanbao Desktop.app` to the Trash, then delete the `~/.hanbao` configuration folder
...
- **Log locations**:
  - Windows: View in the Debug shortcut terminal; app log `%LOCALAPPDATA%\io.agentscope.hanbao.desktop\logs\hanbao-desktop.log`; backend `%USERPROFILE%\.hanbao\desktop.log`
  - macOS: app log `~/Library/Logs/io.agentscope.hanbao.desktop/hanbao-desktop.log`; backend `~/.hanbao/desktop.log`
