...
**频道** = 你和 Hanbao 在「哪里」对话：接钉钉就在钉钉里回，接 QQ 就在 QQ 里回。不熟悉这个词的话可以先看 [项目介绍](./intro)。

配置频道有两种方式：

- **控制台**（推荐）— 在 [控制台](./console) 的 **Control → Channels** 页面，点击频道卡片，在抽屉里启用并填写鉴权信息，保存即生效。
- **手动编辑 `agent.json`** — 在智能体工作区的 `agent.json` 中（如 `~/.hanbao/workspaces/default/agent.json`），将需要的频道设 `enabled: true` 并填好鉴权信息；保存后自动重载，无需重启。
...
7. （可选） **将服务器 IP 加入白名单** — 调用钉钉开放平台 API（如下载用户发送的图片和文件）时需要此配置。在应用设置中进入 **"安全设置→服务器出口 IP"**，添加运行 Hanbao 的机器的公网 IP。可在终端执行 `curl ifconfig.me` 查看公网 IP。若未配置白名单，图片和文件下载将报 `Forbidden.AccessDenied.IpNotInWhiteList` 错误。

### 绑定应用

可以在console前端配置，或者修改智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）。
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）里找到 `channels.dingtalk`，填入对应信息：
...
> - 群聊场景建议显式配置 `robot_code`；留空时 Hanbao 会回退使用 `client_id`。

保存后若服务已运行会自动重载；未运行则执行 `hanbao app` 启动。
...
> 注：**操作顺序**为先配置 App ID/Secret → 启动 `hanbao app` → 再在开放平台配置长连接，如果此处仍显示错误，尝试先暂停 Hanbao 服务并重新启动 `hanbao app`。
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）中找到`channels.feishu`，只需填 **App ID** 和 **App Secret**（在开放平台「凭证与基础信息」里复制）：
...
> 注: **App ID** 和 **App Secret** 信息也可以在Console前端填写，但需重启 Hanbao 服务，才能继续配置长链接的操作。
...
> **获取用户昵称（推荐）**：若希望会话和日志中显示**用户昵称**（如「张三#1d1a」）而非「unknown#1d1a」，需额外开通通讯录只读权限 **以应用身份读取通讯录**（`contact:user.base:readonly`）。未开通时，飞书仅返回 open_id 等身份字段，不返回姓名，Hanbao 无法解析昵称。开通后需重新发布/更新应用版本，权限生效后即可正常显示用户名称。
...
3. 为了使 iMessage 中的信息能被获取，需要 **终端** （或你用来运行 Hanbao 的 app） 和 **消息** 有 **完全磁盘访问权限**（系统设置 → 隐私与安全性 → 完全磁盘访问权限）。
...
4. 填写 iMessage 数据库路径。默认路径为 `~/Library/Messages/chat.db`，若你改过系统路径，请填实际路径。有以下两种填写方案：
   ...
   - 填写智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）：
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）里找到 `channels.discord`，填入对应信息：
...
5. 在**开发管理**中获取**AppID**和**AppSecret**（即 ClientSecret），填入 `agent.json`，方式见下方填写 agent.json。在**IP白名单**中添加一个IP。
   ...
   > **提示：** 如果使用魔搭创空间部署Hanbao，QQ频道的IP白名单应填写：`47.92.200.108`
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）里找到 `channels.qq`，把上面两个值分别填进 `app_id` 和 `client_secret`：
...
**OneBot** 渠道通过**反向 WebSocket** 将 Hanbao 连接到 [NapCat](https://github.com/NapNeko/NapCatQQ)、[go-cqhttp](https://github.com/Mrs4s/go-cqhttp)、[Lagrange](https://github.com/LagrangeDev/Lagrange.Core) 或其他任何兼容 [OneBot v11](https://github.com/botuniverse/onebot-11) 的实现。

与内置 QQ 渠道（使用官方 QQ Bot API，功能受限）不同，OneBot v11 提供**完整 QQ 协议**支持：个人号、群聊无需 @、富媒体消息等。

### 工作原理

Hanbao 启动一个 WebSocket 服务器，OneBot 实现（如 NapCat）作为客户端连接过来：

```
NapCat  ──反向 WS──▶  Hanbao (:6199/ws)
```
...
3. 进入 **网络配置** → **新建** → **WebSocket 客户端**（反向 WS）：
   - URL：`ws://<hanbao地址>:6199/ws`
   - Access Token：与 Hanbao 配置中的 `access_token` 保持一致（可选）
...
> **Docker Compose 提示：** Hanbao 和 NapCat 一起用 Docker Compose 部署时，NapCat 的反向 WS 地址填 `ws://hanbao:6199/ws`（使用服务名）。
...
> **提示：** 语音和视频在渠道层已正确接收，但需要配置 Hanbao 的转写服务（`transcription_provider_type`）才能让 LLM 理解内容。未配置时语音消息显示为占位符。
...
**方法二**在 `agent.json` 填写（如 `~/.hanbao/workspaces/default/agent.json`）
...
| `media_dir`              | string | `~/.hanbao/media` | 媒体文件（图片、文件等）下载目录          |
| `max_reconnect_attempts` | int    | `-1`               | WebSocket 最大重连次数（`-1` = 无限重连） |
...
- **登录方式**：首次使用时扫描二维码授权，Token 自动持久化到本地文件（默认 `~/.hanbao/wechat_bot_token`），后续启动无需重复扫码。
- **消息接收**：通过 HTTP 长轮询（`getupdates`）持续拉取新消息，支持文本、图片、语音（ASR 转录）和文件。
- **消息发送**：通过 `sendmessage` 接口回复用户，当前仅支持文本（iLink API 限制）。

### 扫码登录（推荐通过 Console）

1. 在 Hanbao Web Console 中进入 **设置 → 通道 → 微信个人（iLink）**。
...
也可直接在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）中配置：
...
| `bot_token_file` | string | `~/.hanbao/wechat_bot_token` | Token 持久化路径，下次启动自动读取                  |
| `base_url`       | string | 官方默认地址                  | iLink API 地址，一般留空使用默认值                  |
| `media_dir`      | string | `~/.hanbao/media`            | 接收到的图片、文件保存目录                          |
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）里找到 `channels.telegram`，填入对应信息：
...
3. 在控制台或智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）中配置 **URL** 和 **Token**。
...
2. 模糊匹配订阅和自动推送
   ...
   模糊订阅全server/+/up主题，根据客户端的client_id自动推送到对应的主题，例如客户端向`/server/client_a/up`推送Hanbao处理完后，将会向`/client/client_b/down`推送消息。
...
3. 重定向主题推送
   ...
   消息会根据redirect_client_id属性，推送至 `client/client_b/down`，从而实现跨主题推送。在物联网场景，可以做到以Hanbao为核心，根据个人需求，多设备间自主推送消息。

---

## Matrix

Matrix 频道通过 [matrix-nio](https://github.com/poljar/matrix-nio) 库将 Hanbao 接入任意 Matrix 服务器，支持私聊和群聊房间中的文本消息收发。
...
在 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）中找到 `channels.matrix`：
...
2. 在 Bot 设置中找到 **方式2**，获取 **AppID** 和 **AppSecret**，填入 Hanbao 的频道设置中，点击 **我已操作**。
...
Voice 频道通过 Twilio ConversationRelay 实现电话语音交互，支持语音转文本（STT）、文本转语音（TTS），让用户可以直接拨打电话与 Hanbao 对话。

### 前置要求

1. **Twilio 账号**：从 [Twilio 官网](https://www.twilio.com/) 注册账号并获取凭证
2. **Cloudflare Tunnel**（或其他内网穿透方案）：将本地 Hanbao 服务暴露到公网，供 Twilio 回调使用
...
Twilio 需要通过公网回调 Hanbao 的 Webhook 接口，因此需要将本地服务暴露到公网。
...
配置完成后，直接拨打你购买的 Twilio 电话号码，即可与 Hanbao 进行语音对话：

1. 拨打电话
2. 听到欢迎语后开始说话
3. Hanbao 将语音转文本，调用 Agent 处理
...
| `welcome_greeting`   | string | `"Hi! This is Hanbao. How can I help you?"` | 欢迎语（接通电话后的第一句话）     |
...
SIP 频道让你可以通过标准 SIP 电话或软电话（如 Linphone、MicroSIP、IP 座机）与 Hanbao 进行语音对话。完全在本地网络或私有基础设施上运行，无需云账号或公网 URL。
...
最快的体验方式。Hanbao 会自动启动内置 SIP 注册服务器，无需 Asterisk、FreeSWITCH 或任何外部服务。
...
打开 **http://127.0.0.1:8088/** → **设置 → 模型**：配置模型提供商和 API Key。然后进入 **控制 → 频道 → SIP**：启用，填入 DashScope API Key，点击 **保存**。其他字段全部留空即可 — `sip_server` 留空时 Hanbao 自动启动内置注册服务器，STT/TTS 默认使用 `aliyun`，语音模型自动选择默认音色。

Hanbao 会自动重启 SIP 频道，终端中会看到：

```
[SIP] Built-in SIP registrar started on 0.0.0.0:5060
[SIP] Quickstart: register your softphone to <你的IP>:5060
[SIP] Dial 'sip:agent@<你的IP>:5060' to talk with Hanbao!
```
...
使用 Asterisk、FreeSWITCH 或任意 SIP PBX 作为注册服务器。将 `sip_server` 设为 PBX 地址，Hanbao 注册为 SIP 分机，由 PBX 路由来电。
...
`sip_server` 留空时，Hanbao 自动在 5060 端口启动内置 SIP 注册服务器，agent 自动注册。设置 `sip_server`（如 `"192.168.1.100:5060"`）时，Hanbao 注册到该外部服务器。

### LiveKit 模式配置

生产模式将 SIP/RTP 委托给 LiveKit SIP Server，处理 NAT 穿透、抖动缓冲和编解码协商。Hanbao 作为 AI 参与者加入 LiveKit 房间。
...
3. 启动 Hanbao。如需 SIP 电话呼入，还需部署 LiveKit 基础设施并配置 SIP Trunk 和 Dispatch Rule（参见 [LiveKit SIP 文档](https://docs.livekit.io/sip/)）。浏览器测试请参见上方[快速体验](#快速体验livekit-模式浏览器测试3-分钟无需-sip-电话)。
...
1. 电话接通，听到欢迎语
2. 开始说话 — Hanbao 通过流式 STT 将语音转为文本
3. Agent 处理消息并生成回复
4. 回复通过 TTS 转为语音播放给你
5. 自然地继续对话 — 完全支持多轮对话
...
Azure Bot channel 基于 [Bot Framework](https://dev.botframework.com/) Webhook 协议，支持将 Hanbao 接入 **Microsoft Teams**、**Web Chat**、**DirectLine** 等所有 Azure Bot Service 支持的频道。

配置分为以下几步：先在 **Microsoft Entra ID** 注册应用以获取凭证，再创建 **Azure Bot** 资源并关联已有注册，最后将 Hanbao 的 Webhook 地址填入并启用目标频道。

> **提示**：Azure Bot 是**插件频道**，并非内置频道。使用前请先在 Hanbao 控制台的**插件市场**中搜索并安装 `azure-bot` 插件；安装完成后，该频道才会出现在「频道」设置中。
...
5. 在左侧菜单点击 **"证书和机密（Certificates & secrets）"** → 选择 **"客户端机密（Client secrets）"** 标签 → 点击 **"新建客户端机密（New client secret）"**。
   ...
   填写描述（如 `hanbao`），选择合适的有效期，点击 **"添加（Add）"**。
...
2. 填写基础信息：
   ...
   - **机器人句柄（Bot handle）**：全局唯一，可自定义（如 `hanbao-bot`）
...
Hanbao 会在本地启动一个独立 HTTP 服务（默认端口 `3978`）接收 Azure 转发的消息。Azure Bot Service 要求该端点**可从公网通过 HTTPS 访问**。

**方式 A：固定域名 + 反向代理（推荐生产环境）**

如果 Hanbao 运行在有公网 IP 的服务器上，使用 Nginx 反向代理并配置 HTTPS 证书，Webhook 地址形如：
...
在智能体工作区的 `agent.json`（如 `~/.hanbao/workspaces/default/agent.json`）里找到 `channels.azure_bot`，填入对应信息：
...
- **多频道并行**：同一个 Azure Bot 资源可同时连接 Teams、Web Chat、DirectLine 等多个频道，Hanbao 会自动识别来源频道并路由回复。
- **会话引用持久化**：Hanbao 将各用户 / 群聊的会话引用保存在工作区的 `azure_bot_refs.json` 中，重启后可继续主动向用户发送消息。
...
在代理的 `agent.json` 文件中（例如 `~/.hanbao/workspaces/default/agent.json`）找到 `channels.slack` 部分，填写相关字段：
...
- Hanbao 魔法命令（如 `/stop`、`/model list`）可以作为原生 Slack 斜杠命令发送。也可以作为普通消息发送 — 在线程中发送时加一个空格前缀（如 ` /stop`）即可绕过 Slack 的斜杠命令拦截。
...
4. 使用 `hanbao plugin install <路径>` 安装
...
> `hanbao channels install/add/remove` CLI 命令已被移除。如果你有现存的
...
>    FastAPI `APIRouter`
> 5. 安装插件：`hanbao plugin install <路径>`
