...
The **Console** is Hanbao's built-in web interface. After running `hanbao app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with Hanbao in real time
- Enable/disable/configure messaging channels
- View and manage all chat sessions
- Manage scheduled jobs and heartbeat
- Edit Hanbao's persona and behavior files
- Enable/import skills to extend Hanbao's capabilities
...
This is where you talk to Hanbao. It is the default page when the Console opens.
...
button (↑). Hanbao replies in real time.
...
Create and manage scheduled jobs that Hanbao runs automatically by time.
...
The **simplest way to create a cron job is to chat directly with Hanbao** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message Hanbao on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task type & content** — **Text**: send fixed text from **Message content**.
     **Agent**: fill **Request content**; on each run Hanbao receives the text
...
tick, send the contents of `HEARTBEAT.md` as a user message to Hanbao, and
...
Edit files that define Hanbao's persona and behavior — `SOUL.md`, `AGENTS.md`,
`HEARTBEAT.md`, etc. — directly in the browser.

> **Multi-agent:** Starting from **v0.1.0**, Hanbao supports **multi-agent** mode.
> You can run multiple independent agents in one Hanbao instance, each with its own
...
Manage skills that extend Hanbao (e.g. read PDF, create Word, fetch news). More
...
Manage runtime environment variables needed by Hanbao tools and skills (e.g.
`TAVILY_API_KEY`).
...
> **Note:** Variable validity is your responsibility. Hanbao only stores and loads
> values.
>
...
> Data is stored in `~/.hanbao/token_usage.json`. Override the filename with
> `HANBAO_TOKEN_USAGE_FILE`. See [Config — Environment variables](./config#environment-variables).
...
2. Install the optional Python dependency in the environment that runs Hanbao:
   `pip install "hanbao[whisper]"`.
3. Restart Hanbao, then select **Local Whisper** here.
