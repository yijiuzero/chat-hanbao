...
Hanbao **终端界面（TUI）** 是一个完全运行在终端里的全屏聊天界面。它连接的是与控制台、IM 频道**完全相同**的智能体——同一份记忆、同一套技能、同样的 MCP 工具、同样的会话——但你无需离开键盘。如果你习惯待在终端里，这是与智能体对话、推进任务，或接续在别处开启的会话的最快方式。

它也是 **Coding 模式** 最自然的入口：在某个代码仓库目录中启动，智能体就会把该目录当作它的工作场所。

![Hanbao 终端界面](https://img.alicdn.com/imgextra/i2/O1CN01IULzib1TRAzigIcqG_!!6000000002378-2-tps-2350-1312.png)

---

## 启动方式

终端界面随 `hanbao` 命令行工具一同提供。直接运行不带参数的 `hanbao` 即可打开：

```bash
hanbao                       # 与当前激活的智能体开启对话
hanbao tui                   # 同上，可显式指定选项
hanbao tui --agent NAME      # 与指定的智能体对话
hanbao tui --resume <id>     # 恢复并继续之前的某个会话
```
...
**会话与恢复。** 每段对话都是一个会话，并与 Hanbao 的其余部分共享。用 `/resume` 从建议列表中挑选最近的会话，或用 `/resume list` 浏览全部会话。你也可以在启动时用 `hanbao tui --resume <id>` 直接恢复。
...
其余所有以 `/` 开头的输入都会转发给 Hanbao 智能体——也就是控制台和频道中**同样的魔法命令**，例如 `/model`、`/clear`、`/compact`、`/skills`、`/status`。终端界面会展示所连接智能体支持的全部命令，因此建议列表始终与你的智能体保持一致。完整清单见 [魔法命令](/docs/commands)。
...
终端界面是一个轻量**客户端**。它是一个基于 [Textual](https://textual.textualize.io/) 的终端应用，通过 **ACP**（[Agent Client Protocol](/docs/acp-integration)）驱动本地的 Hanbao 后端。启动时它会以子进程方式拉起 `hanbao acp` 并与之进行 ACP 通信；所有真正的工作——模型调用、记忆、技能、MCP 工具、会话存储——都发生在那个后端里，正是控制台和频道所使用的同一个后端。

<svg viewBox="0 0 760 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Hanbao 终端界面是一个轻量的 Textual 客户端，通过 ACP 以 stdio 子进程方式驱动本地的 hanbao acp 后端。" style="width:100%;height:auto;max-width:700px;display:block;margin:1.5rem auto;font-family:ui-sans-serif,system-ui,sans-serif;">
<defs>
