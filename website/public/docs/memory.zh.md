...
**长期记忆** 让 Hanbao 拥有跨对话的持久记忆能力。默认后端会在 Hanbao 进程内嵌入 ReMe 应用，并通过 ReMe jobs
...
| 能力               | 说明                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------- |
| **嵌入式 ReMe**    | Hanbao 在进程内启动 ReMe，并将当前 Agent 使用的 Hanbao 模型注入到 ReMe 默认 LLM 组件  |
...
| **Inbox 通知**     | `auto_memory`、`auto_dream`、`auto_resource` 产生结果时，会推送到 Hanbao inbox         |
...
Hanbao 的记忆系统采用可插拔的 Backend 架构。除了默认的 ReMeLight（本地文件存储）外，还支持通过 `memory_manager_backend` 切换到其他后端。

### ADBPG（AnalyticDB for PostgreSQL）

基于云端向量数据库的长期记忆后端，适合需要跨设备共享、大规模语义检索的场景。Hanbao 通过 ADBPG 记忆服务的 REST API 接入，无需安装额外数据库驱动。
...
> ⚠️ 切换后端不支持热更新，保存后需要重启 Hanbao 才能生效（页面也会以黄色横幅提醒）。
