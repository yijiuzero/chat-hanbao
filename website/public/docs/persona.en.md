...
Agent persona files are Markdown documents stored in the agent's workspace directory. The workspace location is determined by the `HANBAO_WORKING_DIR` environment variable (defaults to `~/.hanbao`), with the full path being:

```
$HANBAO_WORKING_DIR/workspaces/{agent_id}/
```
...
When running `hanbao init` for the first time, BOOTSTRAP.md is automatically created to guide the initial "conversation" between user and agent, establishing identity, preferences, and style. Once complete, the agent writes the configuration to PROFILE.md and SOUL.md, then deletes BOOTSTRAP.md.
...
You can also directly edit the `system_prompt_files` field in the agent configuration file (`~/.hanbao/workspaces/{agent_id}/agent.json`) to manage persona file loading:
...
Running `hanbao init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`):
...
If using `hanbao init --defaults`, the default language is `zh` (Chinese).
...
Hanbao automatically creates a built-in agent named **"QA Agent"** (ID: `Hanbao_QA_Agent_0.2`) when you first run `hanbao init`.

### QA Agent Features

This is an agent **specifically designed to answer Hanbao-related questions**:

- **Dedicated persona**: Uses persona files optimized for Q&A (different from regular agents)
- **Pre-installed skills**: Auto-enables `guidance` and `QA_source_index` skills for querying Hanbao documentation and source code
...
- **Auto-maintained**: Running `hanbao init` ensures this agent exists

### How to Use?

You can select "QA Agent" in the agent switcher at the top-right of the Console, then ask it any questions about Hanbao.
...
- **Can delete**: Delete it in "Settings → Agent Management" (doesn't affect other agents; will be recreated on next `hanbao init`)
- **Workspace location**: `$HANBAO_WORKING_DIR/workspaces/Hanbao_QA_Agent_0.2/` (defaults to `~/.hanbao/workspaces/Hanbao_QA_Agent_0.2/`)
