...
This document will guide you through using the RESTful API to interact with Hanbao Agents.

> **Protocol Details**: Hanbao's API is based on an extension of the AgentScope Runtime protocol. For more details, see:
> [AgentScope Runtime Protocol Documentation (English)](https://runtime.agentscope.io/en/protocol.html)

> ⚠️ **Security Warning**:
> If your Hanbao instance is **exposed to the public internet**, strongly recommend enabling [Web Login Authentication](./security#web-authentication)!
...
Hanbao provides a RESTful API interface that allows you to interact with Agents via HTTP requests. Through the API, you can:
...
- This is designed for local development and CLI tools (`hanbao`) convenience
...
data: {"sequence_number":2,"object":"response","status":"in_progress","output":[{"role":"assistant","content":[{"type":"text","text":"Hello! I'm Hanbao..."}]}],...}
...
Hanbao automatically manages conversation context through `session_id` and `user_id`. Simply use the same `session_id` across different requests, and the system will automatically save and load conversation history:
...
If [Web Login Authentication](./security#web-authentication) is enabled (`HANBAO_AUTH_ENABLED=true`), all API requests require an authentication token.

#### Register Account

**First-time setup requires registering an admin account** (Hanbao uses single-user mode):
...
```bash
# Delete auth file
rm ~/.hanbao.secret/auth.json

# Or use HANBAO_SECRET_DIR environment variable
rm "${HANBAO_SECRET_DIR}/auth.json"

# Restart Hanbao and re-register
hanbao app
```
...
You can also auto-create an account via environment variables when starting Hanbao:

```bash
export HANBAO_AUTH_ENABLED=true
export HANBAO_AUTH_USERNAME=admin
export HANBAO_AUTH_PASSWORD=admin123
hanbao app
```
...
# Windows (CMD)
set HANBAO_AUTH_ENABLED=
hanbao app

# Windows (PowerShell)
Remove-Item Env:\HANBAO_AUTH_ENABLED
hanbao app
```

**Method 2: Docker Deployment**

Remove the `-e HANBAO_AUTH_ENABLED=true` parameter:
...
Verify Hanbao service is running:
