...
![Hanbao TUI](https://img.alicdn.com/imgextra/i2/O1CN01IULzib1TRAzigIcqG_!!6000000002378-2-tps-2350-1312.png)

---

## Launching

The TUI ships with the `hanbao` CLI. Running `hanbao` with no arguments opens it directly:

```bash
hanbao                       # open a chat with the active agent
hanbao tui                   # same thing, with explicit options
hanbao tui --agent NAME      # chat with a specific agent
hanbao tui --resume <id>     # resume a previous session and continue it
```

To start a **project-bound (Coding Mode)** session, point it at a directory:

```bash
hanbao .                     # use the current directory as the project
hanbao tui /path/to/repo     # use an explicit directory
```
...
> No separate install or server to manage: the TUI launches its own `hanbao acp` backend using the interpreter it ships in, so it always drives the same install/venv as your `hanbao` command.
...
**Sessions & resume.** Every chat is a session shared with the rest of Hanbao. Use `/resume` to pick a recent session from the suggestions, or `/resume list` to browse all of them. You can also resume on launch with `hanbao tui --resume <id>`.
...
<text x="146" y="98" text-anchor="middle" font-size="17" font-weight="700" fill="currentColor">Hanbao TUI</text>
...
<text x="614" y="86" text-anchor="middle" font-size="17" font-weight="700" fill="currentColor">hanbao acp</text>
...
- [CLI](/docs/cli) — other `hanbao` subcommands
