...
- **Working directory:** Edit skill files directly under `$HANBAO_WORKING_DIR`
  (default `~/.hanbao`), including `$HANBAO_WORKING_DIR/skill_pool/` and each
  workspace's `$HANBAO_WORKING_DIR/workspaces/{agent_id}/skills/`.
...
Hanbao skills are organized in two layers:

- **Skill Pool:** Shared local repository at `$HANBAO_WORKING_DIR/skill_pool/`
  (default `~/.hanbao/skill_pool/`).
- **Workspace Skills:** The local runtime copy at
  `$HANBAO_WORKING_DIR/workspaces/{agent_id}/skills/`
  (default `~/.hanbao/workspaces/{agent_id}/skills/`).
...
- **Conflict handling:** If save, import, upload, or broadcast would land on a
  name that already exists, Hanbao returns a conflict instead of silently
...
1. **Import built-ins**.
   ...
   | **QA_source_index**           | Internal Hanbao source/doc index skill for quickly mapping keywords to source paths and local docs.                            | Built-in                                                       |
   | **cron**                      | Scheduled jobs. Create, list, pause, resume, or delete jobs via `hanbao cron` or Console **Control → Cron Jobs**.              | Built-in                                                       |
   ...
   | **guidance**                  | Answer Hanbao installation and configuration questions by consulting local docs first.                                         | Built-in                                                       |
   ...
   **out-of-date**. Use **Update Built-in Skills** to add missing built-ins
   ...
   [CLI](./cli) (`hanbao cron`) or Console **Control → Cron Jobs**:

   - Create: `hanbao cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - List: `hanbao cron list`
   - Check state: `hanbao cron state <job_id>`
...
4. **Manual filesystem changes**.
   You can place folders directly under `$HANBAO_WORKING_DIR/skill_pool/`, but this is not
...
`$HANBAO_WORKING_DIR/skill_pool/`. You can also register one or more **external skill
roots** in the config so Hanbao reads the skills they contain into the **same skill pool
...
Edit `$HANBAO_WORKING_DIR/config.json` and add the top-level `skill_paths` field:
...
`$HANBAO_WORKING_DIR` defaults to `~/.hanbao` and can be overridden with the
`HANBAO_WORKING_DIR` environment variable. See [Config](./config) for the full
configuration reference.

### Workspace Skills

Every workspace runs from its own local copies under
`$HANBAO_WORKING_DIR/workspaces/{agent_id}/skills/`. Those copies are what the agent
...
```bash
hanbao skills install <skill_url>
hanbao skills install <skill_url> --agent-id <agent_id>
```

CLI also supports uninstalling from the shared pool or one workspace:

```bash
hanbao skills uninstall <skill_name>
hanbao skills uninstall <skill_name> --agent-id <agent_id>
```
...
`$HANBAO_WORKING_DIR/workspaces/{agent_id}/skills/`, including using Hanbao itself to help
...
Create a directory under `$HANBAO_WORKING_DIR/workspaces/{agent_id}/skills/`, add a
...
variables, declare them in `metadata.requires`; Hanbao exposes them as
...
workspace and channel, Hanbao injects that config into the runtime environment
...
The full config is always available as `HANBAO_SKILL_CONFIG_<SKILL_NAME>`
...
- `timeout` is not in `requires.env`, so it is only available via the full
  JSON below.
- `HANBAO_SKILL_CONFIG_MY_SKILL` always contains the full JSON config.
...
cfg = json.loads(os.environ.get("HANBAO_SKILL_CONFIG_MY_SKILL", "{}"))
...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` → `metadata.hanbao.requires` → `metadata.requires`. The first one found is used.
