...
Hanbao 新版保留了旧版插件系统的大部分公开 API。旧版公开 API 多数签名保持兼容，可以继续调用；但如果插件依赖 agent 状态、workspace 信息、runtime helper、工具配置结构或前端页面结构，仍需要在新版环境中验证实际行为。
...
- 使用 `window.Hanbao.*` Host SDK 的前端插件
...
因此，把旧版插件原样放到新版时，如果只保留 `"min_version": "1.1.10"`，新版会将它解释为 `>=1.1.10, <1.2.0`，在 Hanbao 2.0.x 下会被判定为不兼容。

### 清单字段说明

| 字段                                                                                                                | 类型     | 旧版                   | 新版                                                  | 迁移建议                         |
| ------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------- | ----------------------------------------------------- | -------------------------------- |
| `hanbao_version`                                                                                                   | `object` | 未定义，会被忽略       | 新增，推荐使用                                        | 新版插件建议增加该字段           |
| `hanbao_version.min`                                                                                               | `string` | 未定义                 | 最低兼容 Hanbao 版本，包含该版本                     | 写为实际验证过的最低新版版本     |
| `hanbao_version.max`                                                                                               | `string` | 未定义                 | 最高兼容 Hanbao 版本，不包含该版本                   | 建议显式填写                     |
| `min_version`                                                                                                       | `string` | 支持，但不用于加载拦截 | 遗留字段，仅在没有 `hanbao_version` 时参与兼容性判断 | 需要兼容旧版时可以保留           |
...
旧版会忽略未知的 `hanbao_version` 字段。新版会优先读取 `hanbao_version`，只有该字段不存在时才回退到 `min_version` / `max_version`。
...
新版继续支持已有的 `window.Hanbao.*` 前端 Host SDK。使用旧版已有前端 API 的插件通常可以直接运行。

| API                                                            | 类型 | 用途                                             | 迁移建议                               |
| -------------------------------------------------------------- | ---- | ------------------------------------------------ | -------------------------------------- |
| `window.Hanbao.host`                                          | 兼容 | 访问 React、Ant Design、API helper、运行时状态等 | 接口保留，需验证 hook 返回值和状态对象 |
| `window.Hanbao.menu`                                          | 兼容 | 注册侧边栏菜单                                   | 接口保留，需验证菜单位置和路由         |
| `window.Hanbao.route`                                         | 兼容 | 注册页面路由                                     | 接口保留，需验证页面加载和卸载         |
| `window.Hanbao.slot`                                          | 兼容 | 注册 UI 插槽                                     | 接口保留，需验证插槽位置               |
| `window.Hanbao.chat.requestPayload.add(pluginId, fn, opts?)`  | 新增 | 在聊天请求发送前追加或改写请求体字段             | 需要改写请求体时使用                   |
| `window.Hanbao.chat.response.set(pluginId, { avatar, nick })` | 新增 | 设置默认 AI 回复卡片的头像和昵称                 | 需要统一回复头像或昵称时使用           |
...
新版插件市场目录会按 Hanbao 版本过滤插件条目。过滤规则与加载器一致。

| 字段                  | 类型     | 读取优先级 | 说明                                      |
| --------------------- | -------- | ---------- | ----------------------------------------- |
| `hanbao_version`     | `object` | 1          | 推荐字段，格式与插件清单一致              |
| `hanbao_version.min` | `string` | 1          | 最低兼容 Hanbao 版本，包含该版本         |
| `hanbao_version.max` | `string` | 1          | 最高兼容 Hanbao 版本，不包含该版本       |
| `min_version`         | `string` | 2          | 旧字段，仅在没有 `hanbao_version` 时使用 |
| `max_version`         | `string` | 2          | 旧字段，仅在没有 `hanbao_version` 时使用 |
| 无版本约束            | -        | 3          | 会被视为兼容，但发布时不建议省略          |

发布新版插件时，建议插件包内的 `plugin.json` 与市场索引条目使用一致的版本约束。如果旧条目只写了 `"min_version": "1.1.10"`，在 Hanbao 2.0.x 下可能会被过滤。

## 迁移步骤

1. 更新 `plugin.json`，增加 `hanbao_version`，并设置明确的 `max`。
2. 确认后端入口导出了 `plugin = MyPlugin()`。
3. 搜索 `register_prompt_section`，将调用改为关键字参数形式，并显式传入 `after`。
4. 如果插件提供 skill，验证用户手动修改开关后的持久化行为。
5. 在新版环境执行插件安装和校验。
6. 启动 Hanbao，检查日志中是否有 `is incompatible` 或插件注册失败信息。
...
可以。插件代码只使用两边都存在的 API，或者在调用新版新增 API 前做版本判断。清单中可以同时保留 `min_version` 和 `hanbao_version`：
