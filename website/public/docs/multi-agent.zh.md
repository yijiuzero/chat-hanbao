...
Hanbao 支持**多智能体**，允许您在同一个 Hanbao 实例中运行多个独立的 AI 智能体。
...
简单来说，**多智能体**就是让您可以在一个 Hanbao 中运行多个"分身"，每个分身：
...
启动 Hanbao 后，在控制台**左上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.hanbao/workspaces/{agent_id}` 目录。
...
如果您之前使用 Hanbao **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
除了与**不同 workspace 的其他 Agent** 协作（`chat_with_agent`），Hanbao 还支持在**当前项目内**派发临时子任务。
...
| 环境                                       | 行为                                                                                                |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| 开启 Coding Mode + project_dir 是 git 仓库 | 在 `<project_dir>/.hanbao/worktrees/` 下创建 **git worktree**，子 Agent 在隔离的 worktree 中工作   |
| 未开启 Coding Mode + workspace 是 git 仓库 | 在 `<workspace_dir>/.hanbao/worktrees/` 下创建 **git worktree**，子 Agent 在隔离的 worktree 中工作 |
...
Hanbao 在创建 worktree 时会自动将这些文件复制过去，确保子任务能正常运行。
