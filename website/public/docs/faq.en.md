...
### Hanbao vs OpenClaw: Feature Comparison
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the Hanbao path**. If you receive the prompt `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - Hanbao path: Typically located at `%USERPROFILE%\.hanbao\bin`.
...
### How to update Hanbao

To update Hanbao, use the method matching your installation type:
...
If you previously used CoPaw, upgrading to Hanbao only requires downloading the latest Hanbao. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start Hanbao service
...
with Hanbao's default port **8088**. This affects all installation methods
...
- Hanbao fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `hanbao app --port 8090` from a terminal instead
...
### APITimeoutError when running Hanbao in WSL2 (NAT mode)

When running Hanbao inside WSL2 with NAT networking (especially when a VPN is
...
After this, Hanbao should be able to communicate with model providers
normally.

### Open-source repository

Hanbao is open source. Official repository:
...
or Hanbao GitHub [Releases](https://github.com/agentscope-ai/QwenPaw/releases).
...
You can also use `hanbao models` for setup, downloads, and switching. See
[CLI → Models and environment variables → hanbao models](https://qwenpaw.agentscope.io/docs/cli#qwenpaw-models).

### How to use Hanbao-Flash series models

Hanbao-Flash is a family of models tuned by the Hanbao team for Hanbao's core
...
Hanbao-Flash models are currently open-sourced on
...
All built-in local providers in Hanbao can be used with Hanbao-Flash models:

#### Hanbao Local (llama.cpp)

In the Hanbao Local model interface, simply choose a Hanbao-Flash model to
...
> Hanbao Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If Hanbao Local does not work properly in your environment, we recommend
> deploying Hanbao-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized Hanbao-Flash model from
   ...
   [Hanbao-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/QwenPaw-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/Hanbao-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
Create a text file named `hanbao-flash.txt` with the following contents. Replace
`/path/to/your/hanbao-xxx.gguf` with the absolute path to the `.gguf` file in
the Hanbao-Flash model repository you downloaded:
...
4. In Hanbao model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized Hanbao-Flash model.
...
4. In Hanbao model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't Hanbao complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a Hanbao bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, Hanbao may show problems such as:
...
- Before running Hanbao, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running Hanbao, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, Hanbao typically needs a
...
The easiest way to create a cron job is to talk to Hanbao in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the Hanbao service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask Hanbao: “Trigger the ‘drink water reminder’ task I just created.”
...
Hanbao supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/QwenPaw/issues) in the Hanbao GitHub
