...
**控制台** 是 Hanbao 内置的 Web 管理界面。运行 `hanbao app` 后，在浏览器中打开
...
这是你和 Hanbao 对话的地方。打开控制台后默认就是这个页面。
...
**发送消息：**
在底部输入框中输入内容，按 **Enter** 或点击发送按钮（↑），Hanbao 会实时回复。
...
创建定时任务的 **最简单的方式是直接与 Hanbao 对话**，让他为你创建。例如你想在钉钉上收到喝水提醒，则在钉钉上与 Hanbao 对话：“帮我创建一个定时任务，每隔 5 分钟提醒我喝水。”创建完成后，可以在控制台的定时任务页面看到创建好的定时任务。
...
为**当前选中的智能体**配置定时「自检」：按间隔把 `HEARTBEAT.md` 里的内容当作用户消息发给 Hanbao，并可把回复投递到指定目标。
...
在这里编辑定义 Hanbao 人设和行为的文件——SOUL.md、AGENTS.md、
HEARTBEAT.md 等——全部在浏览器中完成。

> **多智能体：** 从 **v0.1.0** 开始，Hanbao 支持**多智能体**功能。
> 您可以在同一个 Hanbao 实例中运行多个独立的智能体，每个智能体拥有独立的
...
在这里管理扩展 Hanbao 能力的技能（如读取 PDF、创建 Word 文档、获取新闻等），更详细的内容请看 [Skills](./skills)。
...
在这里管理 Hanbao 的工具和技能在运行时需要的环境变量（如 `TAVILY_API_KEY`）。
...
> **注意：** 环境变量值的有效性需要用户自行保证，Hanbao 只负责存储和加载。
>
...
在聊天中直接问 Hanbao「最近用了多少 token？」或「帮我看看 token 消耗」，Agent 会调用 `get_token_usage` 工具并返回统计结果。

> 数据存储在 `~/.hanbao/token_usage.json`，可通过 `HANBAO_TOKEN_USAGE_FILE` 环境变量自定义文件名。详见 [配置 — 环境变量](./config#环境变量)。
...
1. 使用系统包管理器安装 `ffmpeg`。
2. 在运行 Hanbao 的 Python 环境中安装可选依赖：`pip install "hanbao[whisper]"`。
3. 重启 Hanbao，然后在本页选择 **本地 Whisper**。
