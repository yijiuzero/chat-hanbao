...
本文档说明如何在 Windows 和 macOS 系统上安装和使用 Hanbao Desktop 桌面应用。
...
1. **下载安装包**
   从 [Release 页面][releases]下载 `Hanbao-Tauri-<version>-Windows-setup.exe` 文件

2. **运行安装程序**
   双击 `.exe` 文件，按照安装向导提示完成安装
   - 默认安装位置：`C:\Users\<你的用户名>\AppData\Local\Hanbao Desktop`
...
#### **Hanbao Desktop** (推荐日常使用)

- **特点**: 静默启动，无终端窗口，界面简洁
- **适用场景**: 正常使用，不需要查看技术日志
- **启动方式**: 双击桌面或开始菜单的 "Hanbao Desktop" 图标
- **技术说明**: 原生 Tauri 桌面应用，后台以 sidecar 方式运行 Python 后端

#### **Hanbao Desktop (Debug)** (调试模式)
...
- **适用场景**:
  ...
  - 开发测试
  - 报告 Bug 时需要提供日志
- **启动方式**: 双击开始菜单的 "Hanbao Desktop (Debug)" 图标
- **日志内容**:
  - 应用启动信息
  - Python 错误堆栈
  - API 调用日志
...
A: 使用 "Hanbao Desktop (Debug)" 模式启动，查看终端输出的错误信息

**Q: 如何卸载？**

A: 在 Windows 设置 → 应用 → 已安装的应用 → 找到 "Hanbao Desktop" → 卸载
...
1. **下载压缩包**
   从 [Release 页面][releases]下载 `Hanbao-Tauri-<version>-macOS.zip` 文件

2. **解压缩**
   双击 `.zip` 文件自动解压，得到 `Hanbao Desktop.app` 应用

3. **移动到应用程序文件夹 (可选)**
   将 `Hanbao Desktop.app` 拖到 `/Applications` 文件夹

### 首次启动：解除系统安全限制

#### 为什么需要手动信任？

Hanbao 应用**未经过 Apple 开发者签名和公证（Notarization）**，macOS Gatekeeper 会默认阻止运行。
...
1. **右键点击**（或 Control + 点击）`Hanbao Desktop.app`
2. 在菜单中选择 **"打开"**
...
- 双击 `Hanbao Desktop.app` 即可启动
- 应用会在后台运行，打开应用窗口
- 应用日志输出到：`~/Library/Logs/io.agentscope.hanbao.desktop/hanbao-desktop.log`
- 后端 sidecar 日志位于工作目录：`~/.hanbao/desktop.log`
...
- ✅ 看到完整的 Python 错误堆栈
- ✅ 便于调试和报告问题
- ✅ `HANBAO_DESKTOP_DEBUG=1` 会把桌面日志级别提升为 debug，输出更详细信息
...
1. 检查 `~/.hanbao/desktop.log` 文件查看错误
2. 使用上述终端命令启动，查看实时输出
...
A: 将 `Hanbao Desktop.app` 拖到废纸篓，然后删除 `~/.hanbao` 配置文件夹
...
- **日志位置**:
  - Windows: Debug 快捷方式终端查看；应用日志 `%LOCALAPPDATA%\io.agentscope.hanbao.desktop\logs\hanbao-desktop.log`；后端 `%USERPROFILE%\.hanbao\desktop.log`
  - macOS: 应用日志 `~/Library/Logs/io.agentscope.hanbao.desktop/hanbao-desktop.log`；后端 `~/.hanbao/desktop.log`
