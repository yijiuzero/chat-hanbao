 ...
 *   window.hanbao.registerRoutes("cloudpaw", [{ path: "/a2a", ... }])
 *
...
describe("legacy registerRoutes shim", () => {
  it("attaches window.hanbao.{menu, route, slot, audit}", () => {
    expect(typeof window.hanbao.menu?.add).toBe("function");
    expect(typeof window.hanbao.route?.add).toBe("function");
    expect(typeof window.hanbao.slot?.fill).toBe("function");
    expect(typeof window.hanbao.audit?.overrides).toBe("function");
  });

  it("translates CloudPaw-shape registerRoutes into menu+route", () => {
    window.hanbao.registerRoutes!("cloudpaw", [
      {
        path: "/a2a",
        component: Comp,
        label: "A2A",
  ...
  it("multiple legacy registers share the synthesized plugins-group", () => {
    window.hanbao.registerRoutes!("p1", [
      { path: "/p1", component: Comp, label: "P1", icon: "1" },
    ]);
    window.hanbao.registerRoutes!("p2", [
  ...
  it("new hanbao.menu.add coexists with legacy in plugins-group", () => {
    window.hanbao.registerRoutes!("cloudpaw", [
      { path: "/a2a", component: Comp, label: "A2A", icon: "🔗" },
    ]);
    window.hanbao.menu!.add("newPlugin", {
      id: "newPlugin.foo",
  ...
  it("legacy registerToolRender still writes to pluginSystem", async () => {
    ...
    const RenderFC = () => null;
    window.hanbao.registerToolRender!("p1", { my_tool: RenderFC });
