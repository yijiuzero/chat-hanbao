 ...
 * 2. Plugins access and modify module exports via window.hanbao.modules
...
class ModuleRegistryImpl implements ModuleRegistry {
  ...
  /**
   * Get all modules (for window.hanbao.modules)
   */
...
// Expose to window.hanbao.modules (for plugin use)
// Set during initialization
if (typeof window !== "undefined") {
  if (!window.hanbao) {
    (window as any).hanbao = {};
  }

  // Use Proxy for dynamic access, ensuring plugins always get latest module state
  Object.defineProperty(window.hanbao, "modules", {
    get() {
