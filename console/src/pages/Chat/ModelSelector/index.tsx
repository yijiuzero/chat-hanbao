...
export default function ModelSelector() {
  ...
  const [activeTab, setActiveTab] = useState<"pro" | "free">(
    () =>
      (localStorage.getItem("hanbao_model_selector_tab") as "pro" | "free") ||
      "pro",
  );
  const [collapsedProviders, setCollapsedProviders] = useState<Set<string>>(
    () => {
      try {
        const raw = localStorage.getItem("hanbao_model_selector_collapsed");
  ...
  const toggleProviderCollapse = (providerId: string) => {
    setCollapsedProviders((prev) => {
      ...
      localStorage.setItem(
        "hanbao_model_selector_collapsed",
        JSON.stringify([...next]),
      );
      return next;
    });
    ...
    <div className={styles.panel}>
      ...
      <div className={styles.tabBar}>
        <button
          ...
          onClick={() => {
            setActiveTab("pro");
            localStorage.setItem("hanbao_model_selector_tab", "pro");
          }}
        >
          PRO
        </button>
        <button
          ...
          onClick={() => {
            setActiveTab("free");
            localStorage.setItem("hanbao_model_selector_tab", "free");
          }}
        >
          FREE
        </button>
    ...
    <>
      ...
      >
        <Tooltip title={t("chat.modelSelectTooltip")} mouseEnterDelay={0.5}>
          ...
          >
            {saving && (
              <LoadingOutlined style={{ fontSize: 11, color: "#C0392B" }} />
            )}
