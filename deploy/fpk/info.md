...
- 详细描述：基于 QwenPaw v2.0.1 (Apache-2.0) 减法式二次开发，本地运行、数据自控、仅依赖云端 OpenAI 兼容 API。
- 开源协议：Apache-2.0
- 上游：基于 QwenPaw v2.0.1 (Apache-2.0)，保留上游版权与 NOTICE（合规）
...
- 安装向导可否收集管理员密码并注入 `QWENPAW_AUTH_USERNAME` / `QWENPAW_AUTH_PASSWORD`（I-007 闭环，消除首启抢注窗口）
...
- 展示名已改 hanbao / 函包；包名 `qwenpaw` 全量改名排独立子阶段（暂不影响上架合规）
- 商标提示：应用名 / 图标 / 描述勿用 QwenPaw / AgentScope / Qwen / 通义，勿暗示官方背书
