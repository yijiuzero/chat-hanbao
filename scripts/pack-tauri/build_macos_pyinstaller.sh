#!/usr/bin/env bash
# Build Hanbao with Tauri for macOS (PyInstaller backend)
...
echo "========================================="
echo "Hanbao Tauri Build - macOS (PyInstaller)"
echo "========================================="
...
bash "${SIGN_MACOS_BUNDLE}" \
    "${REPO_ROOT}/console/src-tauri/binaries/hanbao-backend" \
...
APP_PATH="${BUNDLE_DIR}/macos/Hanbao Desktop.app"
if [ ! -d "${APP_PATH}" ]; then
...
# Create ZIP archive
ZIP_NAME="${DIST_ROOT}/Hanbao-Tauri-${VERSION}-macOS.zip"
if [ -f "${ZIP_NAME}" ]; then
    rm -f "${ZIP_NAME}"
fi
...
UPDATER_NAME="${DIST_ROOT}/Hanbao-Tauri-${VERSION}-macOS.app.tar.gz"
case "$(uname -m)" in
