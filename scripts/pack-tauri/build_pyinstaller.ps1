# Build Hanbao backend with PyInstaller for Tauri sidecar (Windows)
...
$VERSION_FILE = "src\hanbao\__version__.py"
...
Write-Host "Hanbao PyInstaller Build - Windows" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
$SPEC_FILE = Join-Path $REPO_ROOT "scripts\pack-tauri\hanbao.spec"
...
# Verify output
$BACKEND_DIR = Join-Path $DIST "pyinstaller\hanbao-backend"
$BACKEND_EXE = Join-Path $BACKEND_DIR "hanbao-backend.exe"
$CLI_EXE = Join-Path $BACKEND_DIR "hanbao.exe"
...
$DEST = Join-Path $BINARIES_DIR "hanbao-backend"
