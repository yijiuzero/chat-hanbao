...
"""
PyInstaller spec file for Hanbao Desktop (Tauri sidecar).
...
bundle also includes a hanbao CLI executable for the Windows installer PATH
option.
"""
...
# from hanbao/console, so Tauri can navigate to the backend-hosted same-origin
...
_data_dirs = [
    ("agents/skills", "hanbao/agents/skills"),
    ("agents/md_files", "hanbao/agents/md_files"),
    ("tokenizer", "hanbao/tokenizer"),
    ("security/tool_guard/rules", "hanbao/security/tool_guard/rules"),
    ("security/skill_scanner/rules", "hanbao/security/skill_scanner/rules"),
    ("security/skill_scanner/data", "hanbao/security/skill_scanner/data"),
    ("app/channels/yuanbao/proto", "hanbao/app/channels/yuanbao/proto"),
]
datas = [
    (str(SRC / src), dst) for src, dst in _data_dirs if (SRC / src).is_dir()
]
datas += collect_tree(CONSOLE_DIST, "hanbao/console")
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        *collect_submodules("hanbao.cli"),
        # All channel adapters (imported on-demand at runtime)
        *collect_submodules("hanbao.app.channels"),
        # ACP runner support is lazily imported by delegate_external_agent.
        *collect_submodules("hanbao.agents.acp"),
        # ASGI app entry points
        "hanbao.app._app",
        "hanbao.app.multi_agent_manager",
        "hanbao.app.chats",
        "hanbao.app.task_tracker",
        "hanbao.runtime.commands",
        # Backup modules are exposed through hanbao.backup.__getattr__, which
        # PyInstaller cannot discover from static imports.
        *collect_submodules("hanbao.backup"),
