#!/usr/bin/env bash
# Build Hanbao backend with PyInstaller for Tauri sidecar
...
DIST="${DIST:-dist}"
VERSION=$(sed -n 's/^__version__[[:space:]]*=[[:space:]]*"\([^"]*\)".*/\1/p' src/hanbao/__version__.py)

echo "========================================="
echo "Hanbao PyInstaller Build"
echo "========================================="
...
SPEC_FILE="${REPO_ROOT}/scripts/pack-tauri/hanbao.spec"
if [ ! -f "$SPEC_FILE" ]; then
...
# Verify output
BACKEND_DIR="${DIST}/pyinstaller/hanbao-backend"
BACKEND_EXE="${BACKEND_DIR}/hanbao-backend"
CLI_EXE="${BACKEND_DIR}/hanbao"
...
DEST="${BINARIES_DIR}/hanbao-backend"
mkdir -p "${DEST}"
find "${DEST}" -mindepth 1 -exec rm -rf {} +
cp -R "${BACKEND_DIR}/." "${DEST}/"
chmod +x "${DEST}/hanbao-backend"
chmod +x "${DEST}/hanbao"
echo "Copied to: ${DEST}"
echo ""
