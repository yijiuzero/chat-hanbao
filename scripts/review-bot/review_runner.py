#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Hanbao AI Review Bot - Main runner script.

This script runs inside GitHub Actions to:
1. Read PR number and repo from environment variables
2. Send a task prompt to the local Hanbao instance
3. Hanbao autonomously fetches PR data via `gh` CLI
...
from hanbao.agents.tools.agent_management import (  # noqa: E402
...
HANBAO_URL = "http://localhost:8088"
CHAT_ENDPOINT = f"{HANBAO_URL}/api/console/chat"
MAX_RETRIES = 3
TIMEOUT_SECONDS = 300
...
def call_hanbao(prompt: str, session_id: str) -> str:
    """Send prompt to Hanbao console chat API and collect SSE response."""
    payload = {
        "channel": "console",
        "user_id": "review-bot",
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(f"[attempt {attempt}/{MAX_RETRIES}] Calling Hanbao...")
            final_event = None
            stream_errors = []
...
def main():
    ...
    print("Sending task to Hanbao (agent will fetch PR data via gh)...")

    response = call_hanbao(prompt, session_id)

    if not response.strip():
        print("\n❌ ERROR: Got empty response from Hanbao")
        sys.exit(1)
