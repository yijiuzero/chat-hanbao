#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Setup Hanbao workspace for the AI Review Bot.

Runs after `hanbao init --defaults --accept-security` to customize
...
WORKING_DIR = Path(
    os.environ.get("HANBAO_WORKING_DIR", Path.home() / ".hanbao"),
)
...
- **Name:** Hanbao Reviewer
- **Role:** AI code reviewer and quality guardian for the Hanbao project
...
You are the lead code reviewer for the Hanbao project. \
...
def harden_governance_policy() -> None:
    ...
    deny_rules = [
        ...
        GovernanceRule(
            match="Bash(*~/.hanbao.secret*)",
            action=GovernanceAction.DENY,
            reason=deny_reason,
        ),
        GovernanceRule(
            match="Bash(*$HOME/.hanbao.secret*)",
            action=GovernanceAction.DENY,
            reason=deny_reason,
        ),
        GovernanceRule(
...
def configure_review_model() -> None:
    ...
    ``hanbao init --defaults`` may pick Hanbao Local (no default model)
