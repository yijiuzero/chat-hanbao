...
unzip -q dist/Hanbao-Tauri-*-macOS.zip -d dist/verify-tauri
...
ps -ef | grep -iE "hanbao|tauri" | grep -v grep || echo "  (no matching processes)"
...
PORT_FILE="$HOME/.hanbao/desktop_port"
PORT=""
for i in $(seq 1 60); do
  ...
  if [ "$i" = "60" ]; then
    ...
    echo "[debug] WORKING_DIR (~/.hanbao) contents:"
    ls -la "$HOME/.hanbao/" 2>/dev/null || echo "  (missing)"
    echo "[debug] All hanbao-related files under HOME (top 30):"
    find "$HOME/.hanbao" -maxdepth 4 -type f 2>/dev/null | head -30 || true
    echo "[debug] desktop.log tail (if exists):"
    tail -50 "$HOME/.hanbao/desktop.log" 2>/dev/null || echo "  (no desktop.log)"
...
rm -f "$HOME/.hanbao/workspaces/default/BOOTSTRAP.md"
