...
$installer = Get-ChildItem dist/Hanbao-Tauri-*-Windows-setup.exe |
...
foreach ($hive in @("HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall",
  ...
  $reg = Get-ChildItem $hive -ErrorAction SilentlyContinue |
    Where-Object { (Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue).DisplayName -match "Hanbao" } |
    Select-Object -First 1
  if ($reg) {
    $loc = (Get-ItemProperty $reg.PSPath).InstallLocation
    if ($loc -and (Test-Path $loc)) {
      $found = Get-ChildItem -Path $loc -Filter "hanbao-desktop.exe" `
...
if (-not $tauriExe) {
  $candidateRoots = @(
    (Join-Path $env:LOCALAPPDATA "Hanbao Desktop"),
    (Join-Path $env:LOCALAPPDATA "Programs\Hanbao Desktop"),
    (Join-Path $env:ProgramFiles "Hanbao Desktop"),
    (Join-Path ${env:ProgramFiles(x86)} "Hanbao Desktop")
  )
  foreach ($root in $candidateRoots) {
    if (Test-Path $root) {
      $found = Get-ChildItem -Path $root -Filter "hanbao-desktop.exe" `
        -Recurse -Depth 3 -ErrorAction SilentlyContinue |
        Select-Object -First 1
      if ($found) { $tauriExe = $found.FullName; break }
    }
...
if (-not $tauriExe) {
  ...
  Write-Host "Registry entries matching Hanbao:"
  foreach ($hive in @("HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall",
                      "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall")) {
...
#    The sidecar writes desktop_port at WORKING_DIR root (~/.hanbao),
#    not inside the workspace dir.
$portFile = Join-Path $env:USERPROFILE ".hanbao\desktop_port"
$port = $null
...
$bootstrapMd = Join-Path $env:USERPROFILE ".hanbao\workspaces\default\BOOTSTRAP.md"
