...
- Builds the console frontend (`console/`), copies `console/dist` to `src/hanbao/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `hanbao:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/hanbao:v1 --no-cache`.
