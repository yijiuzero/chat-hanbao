...
info()  { printf "${GREEN}[hanbao]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[hanbao]${RESET} %s\n" "$*"; }
error() { printf "${RED}[hanbao]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
HANBAO_HOME="${HANBAO_HOME:-$HOME/.hanbao}"
HANBAO_VENV="$HANBAO_HOME/venv"
HANBAO_BIN="$HANBAO_HOME/bin"
PYTHON_VERSION="3.12"
HANBAO_REPO="https://github.com/agentscope-ai/QwenPaw.git"
...
printf "${GREEN}[hanbao]${RESET} Installing Hanbao into ${BOLD}%s${RESET}\n" "$HANBAO_HOME"
...
if [ -d "$HANBAO_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$HANBAO_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$HANBAO_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$HANBAO_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/hanbao/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/hanbao/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/hanbao/console/"*
    fi
}

## Ensure docs are available in src/hanbao/docs/ for source installs.
_DOCS_COPIED=0
prepare_docs() {
    local repo_dir="$1"
    local docs_src="$repo_dir/website/public/docs"
    local docs_dest="$repo_dir/src/hanbao/docs"
...
cleanup_docs() {
    ...
    if [ "$_DOCS_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/hanbao/docs"
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing Hanbao from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        prepare_docs "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$HANBAO_VENV/bin/python" --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
        cleanup_docs "$SOURCE_DIR"
    else
        info "Installing Hanbao from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        ...
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$HANBAO_VENV/bin/python" --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console/cleanup_docs
    fi
else
    PACKAGE="hanbao"
    if [ -n "$VERSION" ]; then
        PACKAGE="hanbao==$VERSION"
    fi
    ...
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$HANBAO_VENV/bin/python" --quiet --index-url "$PYPI_MIRROR" --refresh-package hanbao ${PRERELEASE_ARGS[@]+"${PRERELEASE_ARGS[@]}"}
fi

# Verify the CLI entry point exists
[ -x "$HANBAO_VENV/bin/hanbao" ] || die "Installation failed: hanbao CLI not found in venv"
info "Hanbao installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$HANBAO_VENV/bin/python" -c "import importlib.resources, hanbao; p=importlib.resources.files('hanbao')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$HANBAO_BIN/hanbao" << 'WRAPPER'
#!/usr/bin/env bash
# Hanbao CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

HANBAO_HOME="${HANBAO_HOME:-$HOME/.hanbao}"
REAL_BIN="$HANBAO_HOME/venv/bin/hanbao"
...
chmod +x "$HANBAO_BIN/hanbao"
info "Wrapper created at $HANBAO_BIN/hanbao"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.hanbao/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.hanbao/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}Hanbao installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$HANBAO_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$HANBAO_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}hanbao init${RESET}       # first-time setup\n"
printf "  ${BOLD}hanbao app${RESET}        # start Hanbao\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}hanbao uninstall${RESET}\n"
