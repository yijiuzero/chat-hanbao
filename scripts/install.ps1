# Hanbao Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "dev,whisper"] [-UvPath PATH]
#
# Installs Hanbao into ~/.hanbao with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$HanbaoHome     = if ($env:HANBAO_HOME) { $env:HANBAO_HOME } else { Join-Path $HOME ".hanbao" }
$HanbaoVenv     = Join-Path $HanbaoHome "venv"
$HanbaoBin      = Join-Path $HanbaoHome "bin"
$PythonVersion = "3.12"
$HanbaoRepo     = "https://github.com/agentscope-ai/QwenPaw.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[hanbao] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[hanbao] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[hanbao] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
Hanbao Installer for Windows
...
Environment:
  HANBAO_HOME            Installation directory (default: ~/.hanbao)
"@
    exit 0
}

Write-Host "[hanbao] " -ForegroundColor Green -NoNewline
Write-Host "Installing Hanbao into " -NoNewline
Write-Host "$HanbaoHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\hanbao\console"
...
$VenvHanbao = Join-Path $HanbaoVenv "Scripts\hanbao.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing Hanbao from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\Hanbao"
        }
        Write-Info "Installing Hanbao from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "hanbao-install-$(Get-Random)"
        try {
            git clone --depth 1 $HanbaoRepo $cloneDir
    ...
    $package = "hanbao"
    if ($Version) { $package = "hanbao==$Version" }

    $prereleaseArgs = @()
    if ($Prerelease) { $prereleaseArgs = @("--prerelease=allow") }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --quiet --refresh-package hanbao @prereleaseArgs
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvHanbao)) { Stop-WithError "Installation failed: hanbao CLI not found in venv" }

Write-Info "Hanbao installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, hanbao; p=importlib.resources.files('hanbao')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $HanbaoBin -Force | Out-Null

$wrapperPath = Join-Path $HanbaoBin "hanbao.ps1"
$wrapperContent = @'
# Hanbao CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$HanbaoHome = if ($env:HANBAO_HOME) { $env:HANBAO_HOME } else { Join-Path $HOME ".hanbao" }
$RealBin   = Join-Path $HanbaoHome "venv\Scripts\hanbao.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "Hanbao environment not found at $HanbaoHome\venv"
...
$cmdWrapperPath = Join-Path $HanbaoBin "hanbao.cmd"
$cmdWrapperContent = @"
@echo off
REM Hanbao CLI wrapper — delegates to the uv-managed environment.
set "HANBAO_HOME=%HANBAO_HOME%"
if "%HANBAO_HOME%"=="" set "HANBAO_HOME=%USERPROFILE%\.hanbao"
set "REAL_BIN=%HANBAO_HOME%\venv\Scripts\hanbao.exe"
if not exist "%REAL_BIN%" (
    echo Error: Hanbao environment not found at %HANBAO_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use Hanbao."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "Hanbao installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$HanbaoHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  hanbao init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  hanbao app"  -ForegroundColor White -NoNewline; Write-Host "        # start Hanbao"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "hanbao uninstall" -ForegroundColor White
