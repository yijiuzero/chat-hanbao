...
$Archive = Join-Path $Dist "hanbao-env.zip"
...
$VersionFile = Join-Path $RepoRoot "src\hanbao\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "hanbao-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "hanbao-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "Hanbao Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var HANBAO_LOG_LEVEL or default to "info"
if not defined HANBAO_LOG_LEVEL set "HANBAO_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\hanbao_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.hanbao\config.json" (
  "%~dp0python.exe" -u -m hanbao init --defaults --accept-security
)
"%~dp0python.exe" -u -m hanbao desktop --log-level %HANBAO_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "Hanbao Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with HANBAO_LOG_LEVEL)
if not defined HANBAO_LOG_LEVEL set "HANBAO_LOG_LEVEL=debug"
...
echo ====================================
echo Hanbao Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo PYTHONNOUSERSITE: %PYTHONNOUSERSITE%
echo Log Level: %HANBAO_LOG_LEVEL%
...
echo [Launch] Starting Hanbao Desktop with log-level=%HANBAO_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m hanbao desktop --log-level %HANBAO_LOG_LEVEL%
echo.
echo [Exit] Hanbao Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "Hanbao Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\Hanbao Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create hanbao.cmd wrapper in env root so "hanbao" resolves to this
# instead of Scripts\hanbao.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$HanbaoCmd = Join-Path $EnvRoot "hanbao.cmd"
@"
@"%~dp0python.exe" -u -m hanbao %*
"@ | Set-Content -Path $HanbaoCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('hanbao'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] HANBAO_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "Hanbao-Setup-$Version.exe"
