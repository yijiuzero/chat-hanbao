...
def get_version(manifest: dict[str, Any]) -> dict[str, str] | None:
    """Return a normalized ``hanbao_version`` for CDN metadata.

    Strategy:
      1. If the manifest already provides the structured
         ``hanbao_version`` field, return it directly — the plugin
         explicitly declares its compatibility.
      2. For legacy plugins that only declare ``min_version`` /
         ``max_version``, synthesize a proper ``hanbao_version`` dict
    ...
    hanbao_version = manifest.get("hanbao_version")
...
def _build_metadata(
    ...
    *,
    ...
    if version_constraint:
        metadata["hanbao_version"] = version_constraint
