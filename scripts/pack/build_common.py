...
"""
Create a temporary conda env, install Hanbao from a wheel, run conda-pack.
...
def _pick_wheel(wheel_arg: str | None) -> Path:
    ...
    wheels = sorted(
        (REPO_ROOT / "dist").glob("hanbao-*.whl"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Conda-pack Hanbao (temp env).",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    try:
        ...
        # Install hanbao with all dependencies
        ...
        # hanbao[full]. Restore conda-managed packaging tools before packing.
        _run(
            [
                conda,
                "run",
