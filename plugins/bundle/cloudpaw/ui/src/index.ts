/**
 * CloudPaw frontend plugin for Hanbao
 *
 ...
 *
 * Uses window.Hanbao plugin API (PR #3512+)
 */
...
function buildPlugin() {
  ...
  (window as any).Hanbao.registerToolRender?.("cloudpaw", {
    proposal_choice: ProposalChoiceRender,
    manage_prd: ManagePRDRender,
    a2a_call: A2ACallRender,
  });

  (window as any).Hanbao.registerRoutes?.("cloudpaw", [
    {
      path: "/a2a",
      component: A2APage,
      label: "A2A",
  ...
  // ── Patchable module overrides (Hanbao ≥ 1.1.4b1) ─────────────────
...
function ensureDefaultAgent() {
  const LAST_USED_KEY = "hanbao-last-used-agent";
...
function patchWelcomeAndTheme() {
  const modules = (window as any).Hanbao?.modules;
  if (!modules) return;
