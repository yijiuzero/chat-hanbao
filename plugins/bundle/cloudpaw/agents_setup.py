...
def _build_acp_config(spec: dict[str, Any]) -> Any:
    ...
    """
    try:
        from hanbao.config.config import ACPAgentConfig, ACPConfig
    except ImportError:
...
def _inject_llm_env(env: dict[str, str]) -> None:
    ...
    For iac-code >= 0.1.2: write llm_source: hanbao to settings.yml
    so iac-code reads config directly from Hanbao.
    ...
    _write_hanbao_mode_to_settings()
    return


def _write_hanbao_mode_to_settings() -> None:
    """Write llm_source: hanbao to ~/.iac-code/settings.yml."""
    import yaml
    ...
    # Write llm_source: hanbao
    if settings.get("llm_source") != "hanbao":
        settings["llm_source"] = "hanbao"
        try:
...
def ensure_builtin_agents() -> None:
    ...
    try:
        ...
        from hanbao.config.utils import load_config, save_config
...
def _initialize_agent_workspace(
    ...
    from hanbao.agents.skill_system import get_workspace_skills_dir
...
def _seed_persona_md_files(
    ...
    try:
        from hanbao.agents.prompt import _get_agent_md_dir
...
def _uninstall_plugin_skills() -> None:
    ...
    try:
        from hanbao.agents.skill_system import (
            get_skill_pool_dir,
...
def _install_workspace_skills(
    ...
    try:
        ...
        from hanbao.agents.skill_system.store import (
