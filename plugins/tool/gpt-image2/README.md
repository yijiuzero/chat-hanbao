...
A Hanbao tool plugin that enables image generation and editing using OpenAI's GPT Image 2 model.
...
```bash
hanbao plugin install /path/to/gpt-image2
```

Or from ZIP:

```bash
hanbao plugin install gpt-image2-tool.zip
```

## Configuration

1. Start Hanbao application
...
- Ensure the plugin is installed: `hanbao plugin list`
- Check Hanbao logs: `~/.hanbao/logs/hanbao.log`
- Restart Hanbao after installation
...
- Check file permissions in `~/.hanbao/plugins/`
...
For issues and feature requests, please use the Hanbao issue tracker.
