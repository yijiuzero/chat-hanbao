...
from hanbao.security.tool_guard.safety_checks import (
...
class TestIsCommandDestructive:  # pylint: disable=too-many-public-methods
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "rm -rf /Users/alice/AgentScope/Hanbao/build",
            "rm -rf /home/alice/project/dist",
            "rm -rf /Users/alice/.hanbao/build",
            "rm -rf ~/project",
            "rm -rf ~/project/build",
            "rm -rf ~/project/./*",
            "rm -rf ~alice/project",
    ...
    def test_b1_normalize_before_home_glob_match(self) -> None:
        ...
        assert (
            is_command_catastrophic(
                "rm -rf ../*",
                cwd=os.path.join("/tmp", "hanbao-b1-ws"),
            )
            is False
        )
    ...
    def test_posix_abs_fallback_when_host_resolve_rewrites_drive(
        ...
        import hanbao.security.tool_guard.safety_checks as sc
    ...
    def test_macos_volumes_depth_capped_like_home(self) -> None:
        ...
        assert (
            is_command_catastrophic(
                'rm -rf "/Volumes/My Disk/AgentScope/Hanbao/dist"',
            )
            is False
        )
