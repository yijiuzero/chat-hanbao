...
@pytest.fixture
def mock_config():
    """Patch ``hanbao.config.load_config`` to return a mock
    ...
    with patch("hanbao.config.load_config", return_value=app_cfg) as patched:
        yield patched
...
def mock_env_loader():
    ...
    with patch(
        "hanbao.constant.EnvVarLoader.get_str",
        return_value="",
    ) as patched:
        yield patched
