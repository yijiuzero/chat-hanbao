...
def test_gate_allows_no_project_when_flag_true(
    ...
    _init_repo(other)
    monkeypatch.setattr(
        "hanbao.app.agent_context.get_current_agent_id",
        lambda: "other-agent",
    )
    monkeypatch.setattr(
...
def test_resolve_allowed_fork_project_dir(tmp_path: Path) -> None:
    ...
    wt = project / ".hanbao" / "worktrees" / "abc"
...
def test_mark_fork_failed_waits_for_finalize_lock(
    ...
    from hanbao.agents import fork_project as fp
...
def test_forks_merged_rechecks_registry_after_git(
    ...
    _init_repo(project)
    scope = begin_fork_scope(project)
    wt1 = project / ".hanbao" / "worktrees" / "w1"
    branch1 = "fork/w1"
    ...
    wt2 = project / ".hanbao" / "worktrees" / "w2"
    branch2 = "fork/w2"
    inserted = {"done": False}
...
def test_finalize_lock_released_after_subprocess_crash(
    ...
    child = (
        "import time\n"
        "from pathlib import Path\n"
        "from hanbao.agents.fork_project import _exclusive_file_lock\n"
