...
After running the equivalent of ``hanbao init --defaults --accept-security``
...
def _isolate_app_module_state() -> object:
    """Prevent this module's ``from hanbao.app._app import app`` calls
    from leaking import state into sibling tests.

    ``test_app_factory_importable`` / ``test_critical_routes_return_200``
    import ``hanbao.app._app``, which leaves it in ``sys.modules`` for
    ...
    ``_ensure_hanbao_app_not_loaded()``, which asserts the app module
    ...
    sys.modules.pop("hanbao.app._app", None)
    yield
    # teardown: drop the app factory our tests imported, restore prior
    sys.modules.pop("hanbao.app._app", None)
    if saved_app is not None:
        sys.modules["hanbao.app._app"] = saved_app
...
def isolated_working_dir(
    ...
    """Point HANBAO at a clean tmp working dir for both constant & config.

    ``hanbao.constant.WORKING_DIR`` is read at import time, so we patch the
    already-imported module attribute as well as the consumers that cached it.
    """
    work_dir = tmp_path / "hanbao-home"
    work_dir.mkdir(parents=True, exist_ok=True)

    import hanbao.constant as constant
    ...
    monkeypatch.setenv("HANBAO_WORKING_DIR", str(work_dir))
    return work_dir
...
def test_init_defaults_produces_valid_config(
    ...
    from hanbao.config import Config, get_config_path, save_config
    from hanbao.config.config import AgentsDefaultsConfig, HeartbeatConfig
    from hanbao.constant import HEARTBEAT_DEFAULT_EVERY
...
def test_app_factory_importable() -> None:
    ...
    without raising (the app factory is what ``hanbao app`` serves)."""
    from hanbao.app._app import app  # noqa: F401
...
async def test_critical_routes_return_200() -> None:
    ...
    import httpx
    from hanbao.app._app import app
