# -*- coding: utf-8 -*-
"""Tests for the ``hanbao task`` headless CLI command."""
...
from hanbao.cli.task_cmd import _read_instruction
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("hanbao.cli.task_cmd._run_task", _fake_run_task)
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    async def _spy_run_task(**kwargs):
        ...
        captured["env_tool_guard"] = os.environ.get(
            "HANBAO_TOOL_GUARD_ENABLED",
        )
        captured["env_skills_dir"] = os.environ.get("HANBAO_SKILLS_DIR")
    ...
    monkeypatch.setattr("hanbao.cli.task_cmd._run_task", _spy_run_task)
...
def test_isolated_workspace_creates_overlay(tmp_path):
    ...
    from hanbao.cli.task_cmd import _isolated_skills_workspace
    from hanbao.agents.skill_system import resolve_effective_skills
