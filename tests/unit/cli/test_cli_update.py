...
from hanbao.__version__ import __version__
from hanbao.cli.main import cli
from hanbao.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/hanbao",
...
def test_build_upgrade_command_adds_prerelease_flag_for_uv_only_when_requested(
    ...
    monkeypatch.setattr(
        "hanbao.cli.update_cmd.shutil.which",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "url": "file:///Users/test/Hanbao",
                "dir_info": {"editable": True},
            },
            ("editable", "file:///Users/test/Hanbao"),
        ),
        (
            {
        ...
        (
            {"url": "file:///tmp/hanbao.whl"},
            ("local", "file:///tmp/hanbao.whl"),
        ),
        (
...
def test_detect_installation(
    ...
    from hanbao.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "Hanbao is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("hanbao.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `hanbao update`" in result.output
    assert (
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current Hanbao backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped Hanbao processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `hanbao shutdown` before updating..." in result.output
    assert "Stopped Hanbao processes: 1234" in result.output
    assert "Starting Hanbao update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "hanbao.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "hanbao.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[hanbao] Updating Hanbao 1.0.0 -> 9.9.9..." in captured.out
    assert "[hanbao] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[hanbao] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[hanbao] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()
