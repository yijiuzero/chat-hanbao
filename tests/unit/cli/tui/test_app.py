...
from hanbao.cli.tui.app import PawApp
...
from hanbao.cli.tui.widgets import (
    ActivityLine,
    AgentLabel,
    AssistantMessage,
    CommandMenu,
...
class WarmingTransport(FakeTransport):
    async def start(self) -> Connected:
        ...
        return Connected(
            ...
            hanbao_version=connected.hanbao_version,
            warming=True,
        )
...
@pytest.mark.asyncio
async def test_welcome_message_mounts_with_hanbao_greeting():
    transport = FakeTransport()
    app = PawApp(transport)
    async with app.run_test() as pilot:
        await pilot.pause()
        ...
        assert "Hanbao 9.8.7" not in plain
        ...
        assert "Hanbao 9.8.7" in status
        # The TUI version is no longer shown — only Hanbao's.
...
async def test_terminal_title_uses_session_then_title():
    ...
    async with app.run_test() as pilot:
        ...
        assert titles[-1] == "Hanbao abcd1234"

        await app._dispatch(SessionTitle("Fix the parser"))
        await pilot.pause()
        assert titles[-1] == "Hanbao Fix the parser"
...
async def test_single_agent_label_per_turn_above_thinking():
    ...
    async with app.run_test() as pilot:
        await pilot.pause()
        # thinking → tool → answer: exactly one "hanbao" label, and it sits
        ...
        assert len(labels) == 1
        assert labels[0].content.plain == "hanbao"
...
async def test_model_command_is_forwarded_to_agent():
    """paw no longer manages models — /model is Hanbao's, so forwarded."""
