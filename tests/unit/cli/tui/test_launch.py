# -*- coding: utf-8 -*-
"""Tests for launching the bundled TUI (`hanbao` / `hanbao tui`).
...
Hanbao. The TUI now spawns ``hanbao acp`` using the *current* interpreter.
"""
...
from hanbao.cli.tui import launch
from hanbao.cli.tui.launch import _build_transport, _resolve_workspace_dir
from hanbao.cli.tui.launch import _resume_command
from hanbao.cli.tui.launch import tui_cmd

pytestmark = [pytest.mark.unit, pytest.mark.p1]


def test_default_transport_targets_current_interpreter(tmp_path, monkeypatch):
    """Default spawns this very ``python -m hanbao acp`` (no PATH lookup)."""
    ...
    assert "hanbao acp" in description
...
def test_resume_command_quotes_agent_session_and_project_path(monkeypatch):
    ...
    assert command == (
        "hanbao tui --agent writer --resume 'sess abc' "
...
def test_resume_command_uses_windows_quoting(monkeypatch):
    ...
    assert command == r'hanbao tui --resume "sess abc" "C:\Project Dir"'
...
def test_resume_command_quotes_windows_project_path_with_shell_meta(
    ...
    assert command == r'hanbao tui --resume sess-abc "C:\A&B"'
...
def test_resume_command_quotes_windows_project_path_trailing_backslash(
    ...
    assert command == r'hanbao tui --resume sess-abc "C:\\"'
...
def test_run_tui_prints_resume_hint(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr("hanbao.cli.tui.app.PawApp", FakeApp)
...
def test_run_tui_applies_textual_compat_before_app_runs(monkeypatch):
    ...
    monkeypatch.setattr(
        "hanbao.cli.tui.compat.apply_textual_compat",
...
def test_tui_cmd_invokes_run_tui(monkeypatch):
    ...
    monkeypatch.setattr("hanbao.cli.tui.launch.run_tui", fake_run_tui)
...
def test_bare_hanbao_launches_tui(monkeypatch):
    """Bare ``hanbao`` (no subcommand) opens the TUI."""
...
def test_bare_hanbao_project_launches_tui(monkeypatch):
    """``hanbao .`` opens the TUI with the current directory as project."""
    from hanbao.cli.main import cli
...
def test_bare_hanbao_unknown_command_still_errors(monkeypatch):
    """Only path-like unknown args are treated as TUI project paths."""
    from hanbao.cli.main import cli

    monkeypatch.setattr(
        "hanbao.cli.tui.launch.run_tui",
        pytest.fail,
    )
    result = CliRunner().invoke(cli, ["__missing_hanbao_command__"])
    assert result.exit_code != 0
