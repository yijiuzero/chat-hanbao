...
class TestDetectPlatformMode:
    ...
    @patch("hanbao.sandbox.config.probe_sandbox_support")
...
class TestLinuxSandboxRuleCompilation:
    ...
    def test_basic_workspace_mount(self):
        from hanbao.sandbox.linux_sandbox import _generate_sandbox_script
    ...
    def test_readonly_mount(self):
        from hanbao.sandbox.linux_sandbox import (
            _FS_READ_ACCESS,
            _FS_WRITE_ACCESS,
...
class TestGovernanceSandboxUnavailable:
    ...
    def test_sandbox_fallback_becomes_ask(self):
        ...
        with (
            patch(
                "hanbao.governance.resource_governor.load_governance_policy",
            ) as mock_load,
            patch("pathlib.Path.mkdir"),
            patch(
                "hanbao.governance.resource_governor.probe_sandbox_support",
                return_value=cap,
            ),
        ):
