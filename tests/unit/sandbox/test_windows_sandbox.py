...
class TestProbeSandboxSupport:
    ...
    @patch("hanbao.sandbox.config._probe_windows_appcontainer")
    def test_windows_calls_appcontainer_probe(self, mock_probe):
        from hanbao.sandbox.config import (
            SandboxCapability,
...
class TestProbeAppContainer:
    ...
    def test_non_windows_returns_unsupported(self):
        from hanbao.sandbox.config import _probe_windows_appcontainer
...
class TestACLCommandGeneration:
    ...
    @patch("hanbao.sandbox.windows_sandbox._run_icacls")
    ...
    def test_workspace_gets_full_access(
        ...
        from hanbao.sandbox.windows_sandbox import _apply_all_acls
...
class TestSandboxReuse:
    ...
    def test_save_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            meta_file = containers_dir / "hanbao_test123.json"
            assert meta_file.exists()

            loaded = json.loads(meta_file.read_text(encoding="utf-8"))
            assert loaded["container_name"] == "hanbao_test123"
    ...
    def test_save_metadata_with_acl_manifest(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            containers_dir = state_dir / "containers"
            meta_file = containers_dir / "hanbao_test456.json"
            loaded = json.loads(meta_file.read_text(encoding="utf-8"))
    ...
    @patch("hanbao.sandbox.windows_sandbox._get_appcontainer_sid")
    @patch("hanbao.sandbox.windows_sandbox._get_userenv")
    def test_find_reusable_container_exists(
        ...
        with patch("ctypes.windll", create=True) as mock_windll:
            ...
            result = _find_reusable_container("hanbao_test123")
    ...
    def test_find_reusable_container_not_exists(self, mock_userenv_fn):
        ...
        result = _find_reusable_container("hanbao_nonexistent")
    ...
    def test_find_reusable_container_oserror(self, mock_userenv_fn):
        ...
        result = _find_reusable_container("hanbao_broken")
        assert result is None
...
class TestAppContainerProfileLifecycle:
    ...
    @patch("hanbao.sandbox.windows_sandbox._sid_to_string")
    @patch("ctypes.windll", create=True)
    @patch("hanbao.sandbox.windows_sandbox._get_advapi32")
    @patch("hanbao.sandbox.windows_sandbox._get_userenv")
    ...
    def test_create_profile_already_exists(
        ...
        mock_get_sid.assert_called_once_with("hanbao_existing")
    ...
    def test_delete_profile_success(self, mock_userenv_fn):
        ...
        assert _delete_appcontainer_profile("hanbao_test") is True
    ...
    def test_delete_profile_failure(self, mock_userenv_fn):
        ...
        assert _delete_appcontainer_profile("hanbao_missing") is False
    ...
    def test_delete_profile_oserror(self, mock_userenv_fn):
        ...
        assert _delete_appcontainer_profile("hanbao_err") is False
...
class TestWindowsSandboxExecute:
    ...
    def _make_sandbox(self, **kwargs):
        ...
        sandbox._container_name = "hanbao_test"
        return sandbox

    @patch("hanbao.sandbox.windows_sandbox._wait_and_read_process")
    @patch("hanbao.sandbox.windows_sandbox._create_process_in_appcontainer")
