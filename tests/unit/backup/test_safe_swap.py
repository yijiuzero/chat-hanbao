...
from hanbao.backup._utils._mount_swap import (
...
from hanbao.backup._utils import _mount_swap
from hanbao.backup._utils import safe_swap as safe_swap_module
from hanbao.backup._utils.safe_swap import (
...
def test_normal_directory_uses_rename_swap(secrets_dir: Path) -> None:
    ...
    with patch(
        "hanbao.backup._utils._mount_swap.is_mount_point",
        return_value=False,
    ), patch(
        "hanbao.backup._utils._mount_swap.swap_mount_point_contents",
    ) as mount_swap:
...
def test_mount_point_swap_failure_restores_old_contents(
    ...
    with patch(
        ...
        "hanbao.backup._utils._mount_swap._move_children",
        move_or_fail,
    ), pytest.raises(
        OSError,
...
def test_startup_cleanup_recovers_all_restore_targets(
    ...
    with patch(
        "hanbao.backup._utils.safe_swap._startup_restore_targets",
        return_value=targets,
    ), patch(
        "hanbao.backup._utils.safe_swap.restore_process_lock",
