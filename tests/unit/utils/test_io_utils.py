...
from hanbao.utils.io_utils import (
    append_text_async,
    read_bytes_async,
    read_json_async,
    read_text_async,
...
def test_write_text_atomic_preserves_destination_on_replace_error(
    ...
    with (
        patch(
            "hanbao.utils.io_utils.os.replace",
...
def test_write_text_atomic_temp_is_private_while_writing(
    ...
    with patch("hanbao.utils.io_utils.os.fsync", inspect_fsync):
        write_text_atomic(path, "new", new_file_mode=0o644)
...
def test_write_text_atomic_sets_existing_mode_before_replace(
    ...
    with patch("hanbao.utils.io_utils.os.replace", inspect_replace):
        write_text_atomic(path, "new")
...
def test_write_yaml_atomic_serializes_and_appends_content(
    ...
    assert yaml.safe_load(content) == {"name": "Hanbao"}
...
async def test_async_json_helpers_run_sync_io_in_worker_thread(
    ...
    with (
        patch("hanbao.utils.io_utils.write_json_atomic", fake_write),
        patch("hanbao.utils.io_utils.read_json", fake_read),
    ):
...
async def test_async_json_write_allows_event_loop_progress(
    ...
    with patch(
        "hanbao.utils.io_utils.write_json_atomic",
        delayed_write,
    ):
...
async def test_cancelled_append_holds_lock_until_worker_finishes(
    ...
    with patch("hanbao.utils.io_utils._append_text", delayed_append):
