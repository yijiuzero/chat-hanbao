...
from hanbao.tauri import entry
from hanbao.tauri.env import DESKTOP_CORS_ORIGINS_ENV, DESKTOP_READY_PREFIX
...
def test_ensure_hanbao_app_not_loaded_rejects_late_cors(monkeypatch):
    monkeypatch.setitem(sys.modules, "hanbao.app._app", object())

    with pytest.raises(RuntimeError, match="desktop CORS origins"):
        entry._ensure_hanbao_app_not_loaded()


def test_sync_loaded_hanbao_constant_cors_origins(monkeypatch):
    constant_module = types.SimpleNamespace(CORS_ORIGINS="")
    monkeypatch.setitem(sys.modules, "hanbao.constant", constant_module)
    monkeypatch.setenv(DESKTOP_CORS_ORIGINS_ENV, "tauri://localhost")

    entry._sync_loaded_hanbao_constant_cors_origins()
...
def test_main_supports_frozen_entry_without_package_context(
    ...
    monkeypatch.setattr("hanbao.constant.WORKING_DIR", tmp_path)
    monkeypatch.setattr(
        "hanbao.utils.platform.auto_disable_sandbox_on_windows",
        lambda: calls.append("sandbox-check"),
    )
    monkeypatch.delenv("HANBAO_LOG_LEVEL", raising=False)
