...
def test_structured_hanbao_version_passthrough() -> None:
    manifest = {"hanbao_version": {"min": "1.1.6", "max": "2.1.0"}}
    assert get_version(manifest) == {"min": "1.1.6", "max": "2.1.0"}


def test_structured_hanbao_version_only_min() -> None:
    manifest = {"hanbao_version": {"min": "2.0.0"}}
    assert get_version(manifest) == {"min": "2.0.0"}


def test_structured_hanbao_version_strips_leading_v() -> None:
    manifest = {"hanbao_version": {"min": "v1.0.0", "max": "v2.0.0"}}
    assert get_version(manifest) == {"min": "1.0.0", "max": "2.0.0"}


def test_structured_hanbao_version_strips_whitespace() -> None:
    manifest = {"hanbao_version": {"min": " 1.0.0 ", "max": " 2.0.0\t"}}
    assert get_version(manifest) == {"min": "1.0.0", "max": "2.0.0"}
...
def test_non_dict_hanbao_version_falls_to_legacy() -> None:
    """Non-dict hanbao_version is ignored; falls back to min/max."""
    manifest = {
        "hanbao_version": "invalid",
        "min_version": "1.0.0",
    }
    assert get_version(manifest) == {"min": "1.0.0"}


def test_non_dict_hanbao_version_no_legacy_returns_none() -> None:
    manifest = {"hanbao_version": "invalid"}
    assert get_version(manifest) is None
...
def test_extra_keys_in_hanbao_version_ignored() -> None:
