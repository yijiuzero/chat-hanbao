...
def slack_channel_disabled(
    ...
    from hanbao.app.channels.slack.channel import SlackChannel
...
def slack_event_handler(
    ...
    from hanbao.app.channels.slack.handler import SlackEventHandler
...
class TestSlackConstants:
    ...
    def test_slack_text_limit(self):
        from hanbao.app.channels.slack.constants import SLACK_TEXT_LIMIT

        assert SLACK_TEXT_LIMIT == 30000

    def test_slack_dedup_window_seconds(self):
        from hanbao.app.channels.slack.constants import (
    ...
    def test_slack_reconnect_max(self):
        from hanbao.app.channels.slack.constants import SLACK_RECONNECT_MAX_S

        assert SLACK_RECONNECT_MAX_S == 30.0

    def test_slack_reconnect_factor(self):
        from hanbao.app.channels.slack.constants import SLACK_RECONNECT_FACTOR

        assert SLACK_RECONNECT_FACTOR == 1.8

    def test_slack_reconnect_jitter(self):
        from hanbao.app.channels.slack.constants import SLACK_RECONNECT_JITTER
...
class TestGenerateSessionId:
    def test_thread_session_id(self):
        from hanbao.app.channels.slack.utils import generate_session_id
...
class TestBuildDedupKey:
    def test_uses_event_id_when_present(self):
        from hanbao.app.channels.slack.utils import build_dedup_key
...
class TestDetectFileType:
    def test_png(self):
        from hanbao.app.channels.slack.utils import detect_file_type
...
class TestIsSlackHost:
    def test_slack_dot_com(self):
        from hanbao.app.channels.slack.utils import is_slack_host
...
class TestIsRetryableError:
    def test_429_is_retryable(self):
        from hanbao.app.channels.slack.utils import _is_retryable_error
...
class TestWithRetry:
    ...
    async def test_success_first_try(self):
        from hanbao.app.channels.slack.utils import with_retry
...
class TestProxyResolution:
    def test_explicit_proxy(self, monkeypatch):
        ...
        from hanbao.app.channels.slack.utils import _resolve_slack_proxy_url
    ...
    def test_host_matches_no_proxy_exact(self):
        from hanbao.app.channels.slack.utils import _host_matches_no_proxy
...
class TestIsSlackAngleToken:
    def test_user_mention(self):
        from hanbao.app.channels.slack.format import _is_slack_angle_token
...
class TestEscapeSlackMrkdwn:
    def test_empty_string(self):
        from hanbao.app.channels.slack.format import escape_slack_mrkdwn
...
class TestMarkdownToSlackMrkdwn:
    ...
    def test_empty_string(self):
        from hanbao.app.channels.slack.format import markdown_to_slack_mrkdwn
...
class TestChunkSlackText:
    def test_short_text_single_chunk(self):
        from hanbao.app.channels.slack.format import chunk_slack_text
...
class TestNormalizeSlackThreadTs:
    def test_valid_ts(self):
        from hanbao.app.channels.slack.format import normalize_slack_thread_ts
...
class TestSlackEventHandlerDedup:
    ...
    async def test_dedup_expires(self, slack_event_handler):
        _target = (
            "hanbao.app.channels.slack.handler.SLACK_DEDUP_WINDOW_SECONDS"
        )
...
class TestSlackEventHandlerRichTextExtraction:
    ...
    def test_extract_text_from_blocks_plain(self, slack_event_handler):
        from hanbao.app.channels.slack.handler import (
...
class TestSlackEventHandlerFileExtraction:
    ...
    async def test_get_http_session_uses_channel_proxy(
        ...
        with patch(
            "hanbao.app.channels.slack.handler.aiohttp.ClientSession",
        ) as mock_session_cls:
    ...
    async def test_download_slack_file_retries_on_5xx(
        ...
        with (
            ...
            patch(
                "hanbao.app.channels.slack.handler.asyncio.sleep",
                AsyncMock(),
            ),
        ):
            slack_channel._media_dir = tmp_path / "media"
...
class TestIsSlackSsrfAllowed:
    def test_allowed_slack_com(self):
        from hanbao.app.channels.slack.sender import _is_slack_ssrf_allowed
...
class TestResolveLocalFilePath:
    def test_file_protocol_exists(self, tmp_path):
        from hanbao.app.channels.slack.sender import _resolve_local_file_path
...
class TestSlackSenderRouteResolution:
    def test_compound_handle(self):
        from hanbao.app.channels.slack.sender import SlackSender
...
class TestSlackSenderTextSending:
    ...
    async def test_get_http_session_uses_channel_proxy(
        ...
        sender = SlackSender(channel=slack_channel)
        with patch(
            "hanbao.app.channels.slack.sender.aiohttp.ClientSession",
        ) as mock_session_cls:
            mock_session_cls.return_value.closed = False
...
class TestSlackSenderMediaUpload:
    ...
    async def test_upload_remote_media_allowed(
        ...
        sender = SlackSender(channel=slack_channel)
        with patch(
            "hanbao.app.channels.slack.sender._is_slack_ssrf_allowed",
            return_value=True,
        ):
...
class TestSlackChannelFromConfig:
    def test_from_config(self, mock_process):
        from hanbao.app.channels.slack.channel import SlackChannel
        from hanbao.config.config import SlackConfig
...
class TestSlackChannelNonRecoverableErrors:
    def test_is_non_recoverable_slack_error_matches(self):
        from hanbao.app.channels.slack.channel import (
...
class TestSlackChannelProxyResolution:
    ...
    async def test_proxy_resolution_during_init(self, slack_channel):
        ...
        with patch(
            "hanbao.app.channels.slack.channel._resolve_slack_proxy_url",
...
class TestSlackChannelSocketModeResilience:
    ...
    async def test_restart_socket_mode_first_attempt_delay_positive(
        ...
        slack_channel._start = AsyncMock()
        with patch(
            "hanbao.app.channels.slack.channel.asyncio.sleep",
        ) as mock_sleep:
            await slack_channel._restart_socket_mode("test")
...
class TestSlackEventHandlerUnfurl:
    ...
    def test_append_unfurl_with_title_and_link(self):
        from hanbao.app.channels.slack.handler import _append_unfurl_text
...
class TestSlackEventHandlerUserResolution:
    ...
    async def test_user_name_cache_is_lru_bounded(
        ...
        from hanbao.app.channels.slack import handler as handler_module
...
class TestSlackHandlerExtractMessageText:
    ...
    def test_strips_bot_prefix(self):
        from hanbao.app.channels.slack.handler import _extract_message_text
