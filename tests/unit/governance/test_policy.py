...
from hanbao.governance.audit import AuditLog
from hanbao.sandbox import SandboxCapability
...
def _make_governor(tmp_path) -> ResourceGovernor:
    ...
    (not the real ~/.hanbao), so tests never pollute the home dir."""
...
class TestDefaultPolicyLoad:
    ...
    def test_new_default_rule_auto_migrated(self, tmp_path):
        ...
        with patch(
            "hanbao.governance.policy.DEFAULT_USER_RULES",
            patched,
        ):
...
def _patch_model(monkeypatch, text: str, delay: float = 0.0) -> None:
    ...
    import hanbao.agents.model_factory as factory
...
class TestGeneralizeRuleMatch:
    ...
    async def test_shell_generalizes(self, monkeypatch):
        from hanbao.governance.generalize import generalize_rule_match
    ...
    async def test_timeout_falls_back(self, monkeypatch):
        from hanbao.governance import generalize as policy_mod
...
class TestAddApprovedRuleGeneralization:
    ...
    async def test_concurrent_policy_writes_are_serialized(
        ...
        from hanbao.governance import resource_governor as governor_module
...
class TestGeneralizeTargetForApproval:
    ...
    async def test_builtin_source_returns_exact_target(self, monkeypatch):
        from hanbao.governance.generalize import (
...
class TestDeepScanConfigMerge:
    ...
    def test_deep_scan_disabled_rules_filter(
        ...
        from unittest.mock import MagicMock
        from hanbao.governance.policy import DetectionRuleConfig
...
class TestRawParamsScanning:
    ...
    def test_write_content_match(self):
        ...
        from hanbao.governance.detectors import detect_dangerous_patterns
        from unittest.mock import MagicMock
    ...
    def test_shell_evasion_config_can_disable(self):
        ...
        import hanbao.config

        original = hanbao.config.load_config
        hanbao.config.load_config = lambda: mock_cfg
        try:
            tc = _tc("Bash", "echo $(whoami)")
            findings = policy._deep_security_scan(tc, "shell")
            ...
            hanbao.config.load_config = original
...
class TestDefaultDetectionRules:
    ...
    def test_bundled_rules_loaded_from_empty_policy_yaml(self, tmp_path):
        ...
        from hanbao.governance.policy import _get_default_detection_rules
    ...
    def test_missing_yaml_graceful_degradation(self, monkeypatch):
        ...
        import hanbao.governance.policy as pol
