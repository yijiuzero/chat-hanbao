...
def _write_integration_subprocess_rc(root: Path, dest_ini: Path) -> None:
    ...
    """
    src_hanbao = (root / "src" / "hanbao").resolve()
    text = (
        "[run]\n"
        "parallel = true\n"
        "branch = false\n"
...
class AppServer:
    ...
    # Working directory of the subprocess (= HANBAO_WORKING_DIR). Tests that
...
def app_server(  # pylint: disable=too-many-statements,too-many-branches
    ...
    """Start one isolated hanbao app process per test module.
    ...
    env["HANBAO_WORKING_DIR"] = str(working_dir)
    env["HANBAO_SECRET_DIR"] = str(secret_dir)
    env["HANBAO_BACKUP_DIR"] = str(backups_dir)
    env["HANBAO_AUTH_ENABLED"] = "false"
    ...
    env["HANBAO_UPLOAD_MAX_SIZE_MB"] = "10"
    ...
    env["HANBAO_RUNNING_IN_CONTAINER"] = "true"
    env["NO_PROXY"] = "*"
    ...
    if _integration_coverage_requested():
        if _INTEGRATION_COVERAGE_DIR is None:
            raise AssertionError(
                "HANBAO_INTEGRATION_COVERAGE is set but coverage dir was not "
        ...
        try:
            ...
            while time.time() - start_at < max_wait_seconds:
                if process.poll() is not None:
                    raise AssertionError(
                        "hanbao app exited during startup.\n"
                ...
                raise AssertionError(
                    "hanbao core agents did not become ready in time.\n"
