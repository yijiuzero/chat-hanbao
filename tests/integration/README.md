...
HTTP smoke tests that exercise the Hanbao FastAPI app end-to-end via a
real subprocess. Each test file owns its own Hanbao app subprocess on a
...
test file gets its own Hanbao app subprocess on a random port, sharing
...
- Forces `HANBAO_AUTH_ENABLED=false` and `NO_PROXY=*`
...
```bash
HANBAO_INTEGRATION_COVERAGE=1 pytest tests/integration/ --no-cov
```
