...
from hanbao.app.approvals.driver_gate import HanbaoDriverApprovalGate
from hanbao.app.approvals.service import ApprovalService
from hanbao.app.driver_config_watcher import DriverConfigWatcher
from hanbao.app.mcp.config_service import MCPConfigService
from hanbao.app.mcp.schemas import (
    MCPAccessPolicy,
    MCPAccessRule,
    MCPToolDefaultPolicy,
)
from hanbao.drivers.capabilities import DriverInvocation
from hanbao.drivers.contracts import (
    DriverCard,
    PolicyRule,
...
from hanbao.drivers.storage import card_path, dump_card, load_card
...
async def _registry_with_policy(
    ...
    manager = DriverManager(
        tmp_path / "drivers",
        store,
        approval_gate=HanbaoDriverApprovalGate(),
    )
...
async def test_driver_mcp_policy_ask_approve_resumes_client_call(
    ...
    service = ApprovalService()
    monkeypatch.setattr(
        "hanbao.app.approvals.get_approval_service",
        lambda: service,
    )
    manager = await _registry_with_policy(
        tmp_path,
...
async def test_driver_mcp_policy_ask_active_agent_off_auto_allows(
    ...
    monkeypatch.setattr(
        "hanbao.config.utils.load_config",
