...
def _github_cache_ttl() -> float:
    raw = EnvVarLoader.get_str("HANBAO_GITHUB_CACHE_TTL", "")
    if raw:
        try:
...
def _hub_http_timeout() -> float:
    raw = EnvVarLoader.get_str("HANBAO_SKILLS_HUB_HTTP_TIMEOUT", "30")
    try:
...
def _hub_http_retries() -> int:
    raw = EnvVarLoader.get_str("HANBAO_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = EnvVarLoader.get_str("HANBAO_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = EnvVarLoader.get_str("HANBAO_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_version_path() -> str:
    return EnvVarLoader.get_str(
        "HANBAO_SKILLS_HUB_VERSION_PATH",
...
def _build_async_client() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        ...
        headers={"User-Agent": "hanbao-skills-hub/1.0"},
        limits=httpx.Limits(
...
def _extract_hanbao_skill_spec(
    url: str,
) -> tuple[str, str, str] | None:
    """Parse a Hanbao plaza skill URL into (owner, skill_name, version)."""
    parsed = urlparse(url)
    host = (parsed.netloc or "").lower()
...
def _extract_aliyun_skill_spec(url: str) -> str | None:
    ...
    Accepts URLs synthesised by `hanbao.market.providers.aliyun`, of the
...
async def _fetch_bundle_from_hanbao_url(
    bundle_url: str,
    requested_version: str,
) -> tuple[Any, str]:
    spec = _extract_hanbao_skill_spec(bundle_url)
    if spec is None:
        raise ConfigurationException(
            config_key="skills_hub.bundle_url",
            message="Invalid Hanbao URL format. Use URL like "
    ...
    try:
        ...
        raise SkillsError(
            message=(
                "Hanbao archive download failed: "
                f"{_format_http_error_body(e)}."
            ),
        ) from e
    return (
...
async def _fetch_bundle_from_aliyun_url(
    ...
    try:
        from hanbao.market.providers.aliyun import (
...
PROVIDERS: list[tuple[InstallOrigin, _ProviderMatcher, _ProviderFetcher]] = [
    ...
    (
        "hanbao",
        _extract_hanbao_skill_spec,
        _fetch_bundle_from_hanbao_url,
    ),
    (
        "modelscope",
