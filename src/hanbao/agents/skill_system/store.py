...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "hanbao", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.hanbao.emoji."""
    if not isinstance(metadata, dict):
        return ""
    hanbao = metadata.get("hanbao")
    if isinstance(hanbao, dict):
        return str(hanbao.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="hanbao_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"hanbao_skill_stage_{skill_name}_"),
    )
