...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use hanbao agents chat for two-way communication with replies.
metadata:
...
```bash
hanbao agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `hanbao agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `hanbao agents chat --background` |
...
2. hanbao agents list
3. hanbao agents chat to start a conversation
...
2. hanbao agents list
3. hanbao agents chat --background to submit the task
...
`hanbao agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  hanbao agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && hanbao agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
hanbao agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && hanbao agents chat --background --task-id <id>
sleep 20 && hanbao agents chat --background --task-id <id>
sleep 30 && hanbao agents chat --background --task-id <id>
```
...
```bash
hanbao agents -h
hanbao agents list -h
hanbao agents chat -h
```
