---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage jobs with hanbao cron list/create/get/state/update/pause/resume/delete/run, and always pass --agent-id explicitly.
metadata:
...
All `hanbao cron` commands **must** include:
...
```bash
# List tasks
hanbao cron list --agent-id <agent_id>

# View task details
hanbao cron get <job_id> --agent-id <agent_id>

# View task status
hanbao cron state <job_id> --agent-id <agent_id>

# Create a task
hanbao cron create --agent-id <agent_id> ...

# Delete a task
hanbao cron delete <job_id> --agent-id <agent_id>

# Pause / Resume a task
hanbao cron pause <job_id> --agent-id <agent_id>
hanbao cron resume <job_id> --agent-id <agent_id>

# Run an existing task once immediately
hanbao cron run <job_id> --agent-id <agent_id>

# Update an existing task
hanbao cron update <job_id> --agent-id <agent_id> ...
```
...
```bash
hanbao cron create --agent-id <agent_id> -f job_spec.json
```
...
5. Create the task with hanbao cron create
...
- Before modifying/pausing/deleting, run `hanbao cron list --agent-id <agent_id>` first
- To troubleshoot issues, use `hanbao cron state <job_id> --agent-id <agent_id>`
...
- Before creating, you can run `hanbao chats list --agent-id <agent_id>` to get valid `target-user` and `target-session`
...
hanbao cron -h
hanbao cron list -h
hanbao cron create -h
hanbao cron get -h
hanbao cron state -h
hanbao cron pause -h
hanbao cron resume -h
hanbao cron delete -h
hanbao cron run -h
```
