...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with hanbao chats list, then push with hanbao channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `hanbao channels send`**
...
```bash
hanbao chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
hanbao chats list --agent-id <your_agent> --user-id <user_id>
```
...
`hanbao channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`hanbao channels send` only sends — it does not wait for a reply.
...
```bash
hanbao chats list --agent-id notify_bot --channel feishu
...
```bash
hanbao chats list --agent-id task_bot --channel console
...
```bash
hanbao chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `hanbao channels send`.
...
```bash
hanbao chats list --agent-id <your_agent>
```
...
```bash
hanbao channels list --agent-id <your_agent>
```
...
- **hanbao agents chat**: sends to another agent, two-way, returns a reply
- **hanbao channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `hanbao agents chat`
- Need to proactively push a message to a user → `hanbao channels send`
