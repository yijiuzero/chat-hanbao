# -*- coding: utf-8 -*-
"""Embedded ReMe application configuration for Hanbao memory.

ReMe's standalone CLI normally loads YAML such as
``reme/config/default.yaml`` or ``reme/config/hanbao.yaml``.  Hanbao embeds
...
from hanbao.config.config import AgentProfileConfig, EmbeddingModelConfig
...
def build_reme_app_config(
    *,
    ...
    """Build ReMe ``Application`` kwargs for embedded Hanbao usage."""
...
def _base_config() -> dict[str, Any]:
    """Return the ReMe config shape used by Hanbao."""
...
def _apply_embedding_config(
    ...
    """Map Hanbao embedding config into ReMe component config."""
