...
subclassing the agent, it is injected into :class:`HanbaoAgent` and drives the
two delegated hooks.
...
class ScrollContextManager:
    ...
    def _recall_page_stub(
        ...
        page = cls._block_metadata(block).get("hanbao_recall_page", {})
        next_cursor = (
    ...
    def _is_continuation_stub(msg: Any) -> bool:
        ...
        if not isinstance(metadata, dict):
            return False
        tag = metadata.get(HANBAO_MESSAGE_TAG_KEY)
