...
hanbao envelope schema and the ACP transport stack.  Eagerly importing
them here would slow down every consumer of ``hanbao.agents.acp`` —
including ``delegate_external_agent`` which only needs
``tool_adapter`` — so we expose ``HanbaoACPAgent`` /
``run_hanbao_agent`` / ``ACPService`` / ``*_acp_service`` via
...
_LAZY_EXPORTS = {
    "HanbaoACPAgent": ".server",
    "run_hanbao_agent": ".server",
    "ACPService": ".service",
...
# fine — verified by ``from hanbao.agents.acp import ACPService`` working.
__all__ = [
    "ACPErrors",
    "ACPConfigurationError",
    "ACPProtocolError",
