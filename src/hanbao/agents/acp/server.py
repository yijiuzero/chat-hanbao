# -*- coding: utf-8 -*-
"""Hanbao ACP Agent server.

Exposes Hanbao as an ACP-compliant agent that external clients
...
ACP_ERROR_META_KEY = "hanbao.error"
ACP_AGENT_META_KEY = "hanbao.agent"

_GENERIC_PROMPT_ERROR = (
    "Hanbao failed to process the request. Check server logs for details."
)
...
class HanbaoACPAgent(Agent):
    ...
    def _session_info(
        *,
        ...
        info: dict[str, Any] = {
            ...
            "mode": HanbaoACPAgent.MODE_DEFAULT,
        }
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        async with self._workspace_lock:
            ...
            logger.info(
                "Hanbao ACP Agent workspace started:"
    ...
    async def _bridge_approval_requests(
        ...
        *,
        ...
        """Bridge Hanbao ApprovalService waits to ACP permission prompts."""
    ...
    async def _request_approval_decision(
        ...
        """Ask the ACP client to approve/deny a Hanbao pending approval."""
        from ...app.approvals import get_approval_service
    ...
    def _approval_options(pending: Any) -> list[PermissionOption]:
        """Build ACP permission options for a pending Hanbao approval."""
        display = HanbaoACPAgent._approval_display(pending)
        if (
    ...
    def _session_meta(self) -> dict[str, Any] | None:
        """Return session ``_meta`` with the resolved Hanbao agent id."""
        try:
            agent_id = self._resolve_agent_id()
        except Exception:
...
async def run_hanbao_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
    local_diagnostics: bool = False,
) -> None:
    """Entry point: run Hanbao as an ACP agent over stdio."""
    agent = HanbaoACPAgent(
        agent_id=agent_id,
