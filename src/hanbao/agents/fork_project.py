...
WORKTREE_REL = Path(".hanbao") / "worktrees"
REGISTRY_REL = Path(".hanbao") / "fork_registry.json"
...
INTEGRATION_PROJECT_REL = Path(".hanbao") / "fork_integration_project"
ACTIVE_SCOPE_REL = Path(".hanbao") / "fork_active_scope"
...
def project_dir_from_worktree(worktree_path: Path) -> Path:
    """``<project>/.hanbao/worktrees/<id>`` → ``<project>``."""
...
def _fork_finalize_lock_path(project_dir: Path, branch: str) -> Path:
    ...
    return project_dir / ".hanbao" / "fork_locks" / f"{digest}.lock"
...
def _ensure_git_identity(wt: Path) -> None:
    ...
    Disabled when ``HANBAO_FORK_AUTO_GIT_IDENTITY=0``. Prefers existing
    repo/global git config and does not overwrite a configured identity.
    """
    if os.environ.get("HANBAO_FORK_AUTO_GIT_IDENTITY", "1") == "0":
        return
    for key, value in (
        ("user.email", "hanbao-fork@localhost"),
        ("user.name", "Hanbao Fork"),
    ):
