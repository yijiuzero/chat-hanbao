...
def _keyring_account() -> str:
    ...
    via ``HANBAO_WORKING_DIR``) would therefore share — and silently
    overwrite — the stable install's master key, leaving the stable install
    unable to decrypt its own secrets.

    Resolution:

    1. Explicit ``HANBAO_KEYRING_ACCOUNT`` override always wins (useful for
    ...
    relocated = bool(
        EnvVarLoader.get_str("HANBAO_WORKING_DIR")
        or EnvVarLoader.get_str("HANBAO_SECRET_DIR"),
    )
    if not relocated:
...
def _should_skip_keyring() -> bool:
    ...
    if EnvVarLoader.get_bool("HANBAO_DISABLE_KEYRING"):
        return True
