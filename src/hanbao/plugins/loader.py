...
def _desktop_python() -> Optional[str]:
    ...
    path = os.environ.get("HANBAO_DESKTOP_PY_RUNTIME", "").strip()
...
def _ensure_plugin_site_on_path() -> None:
    ...
    os.environ["HANBAO_PLUGIN_SITE"] = site_dir
...
class PluginLoader:
    ...
    def _check_version_compatibility(
        ...
        """Check plugin compatibility with current Hanbao version.

        Uses left-closed, right-open semantics: ``>=min, <max``.
        When ``hanbao_version`` is absent, falls back to legacy
    ...
    async def _load_backend_module(
        ...
        try:
            ...
            if manifest.hanbao_version is not None:
                qv_dict = manifest.hanbao_version.model_dump()
            else:
                qv_dict = {
    ...
    def _install_requirements(
        ...
        the case for uv-managed venvs created by the Hanbao script
    ...
    def _install_requirements_frozen(
        ...
        if python is None:
            raise RuntimeError(
                ...
                "(HANBAO_DESKTOP_PY_RUNTIME not set). Reinstall Hanbao "
    ...
    async def unload_plugin(
        ...
        removes the plugin's tools from ``hanbao.agents.tools``.
    ...
    def _cleanup_plugin_tools(
        ...
        try:
            ...
            tools_module = sys.modules.get("hanbao.agents.tools")
