...
def get_hanbao_version() -> str:
    """Return the installed Hanbao package version, or ``'unknown'``."""
    try:
...
def generate_backup_id() -> str:
    ...
    Format: ``hanbao-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``hanbao-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_hanbao_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"hanbao-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.hanbao_version = get_hanbao_version()
