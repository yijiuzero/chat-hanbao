...
variable ``HANBAO_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``HANBAO_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = EnvVarLoader.get_str("HANBAO_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``HANBAO_AUTH_ENABLED``
    is truthy and both ``HANBAO_AUTH_USERNAME`` and ``HANBAO_AUTH_PASSWORD``
    ...
    username = EnvVarLoader.get_str("HANBAO_AUTH_USERNAME", "").strip()
    password = EnvVarLoader.get_str("HANBAO_AUTH_PASSWORD", "").strip()
