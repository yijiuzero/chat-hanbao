...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        message = (
            "Hanbao startup failed because restore artifact cleanup did not "
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            ...
            from hanbao.runtime.commands.control import register_command
    ...
    try:
        ...
        # These two cleanup tasks are independent; run in parallel.
        # [hanbao modification] browser_use / Chromium 已移除，不再需要
        # 在关闭时停止浏览器（stop_all_browsers 来自已删除的 browser_control）。
...
_CONSOLE_STATIC_ENV = "HANBAO_CONSOLE_STATIC_DIR"
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get(
        "/{full_path:path}",
        name="hanbao_console_spa_catchall",
    )
