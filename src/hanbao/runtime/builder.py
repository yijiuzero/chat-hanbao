...
:class:`AgentBuilder` fully constructs a :class:`HanbaoAgent` for each
request.  It obtains tools from the per-workspace
:class:`HanbaoLocalWorkspace` (via ``list_tools``), the system prompt
...
class AgentBuilder:
    ...
    async def build_toolkit(
        ...
        *,
        ...
        :class:`HanbaoLocalWorkspace` via ``list_tools()``.
    ...
    async def build(  # pylint: disable=too-many-statements,too-many-branches
        ...
        """Construct a fully-wired :class:`HanbaoAgent` for one request.

        Integrates all per-workspace registries: HanbaoLocalWorkspace
        ...
        from ..agents.react_agent import HanbaoAgent
        ...
        agent = HanbaoAgent(
            name=agent_config.name or "Hanbao",
            model=model,
    ...
    def _build_context_config(agent_config: Any) -> Any:
        """Map Hanbao's ``ContextCompactConfig`` to AS ``ContextConfig``."""
        ...
        try:
            ...
            # creates a replacement ToolResultBlock, and drops Hanbao's
    ...
    def _scroll_recall_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        which requires BOTH the ``HANBAO_ALLOW_UNSANDBOXED_RECALL`` env var
    ...
    def _scroll_repl_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        ``HANBAO_ALLOW_UNSANDBOXED_RECALL`` env var and
