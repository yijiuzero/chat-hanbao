...
class GuardedFunctionTool:
    """Permission-checked tool wrapper using hanbao's tool-guard engine.

    Routes every tool invocation through
    ``hanbao.security.tool_guard.engine`` which evaluates guard rules and
...
async def _guarded_tool_check_permissions(
    ...
    **_extra_kwargs: Any,
    ...
    """Drive hanbao's tool-guard engine + ApprovalService for one tool call.
    ...
    """
    del context  # hanbao's guard doesn't read PermissionContext yet
