...
def _get_env(key: str, default: str = "") -> str:
    ...
    ``HANBAO_``, the corresponding ``COPAW_`` variant is transparently
    checked as a fallback so that existing deployments keep working.
    """
    if key in os.environ:
        return os.environ[key]
    if key.startswith("HANBAO_"):
        legacy_key = "COPAW_" + key[len("HANBAO_") :]
...
class EnvVarLoader:
    ...
    and defaults.  Pass HANBAO_* keys; COPAW_* legacy variants are
...
# 1. HANBAO_WORKING_DIR / COPAW_WORKING_DIR env var is set → use it
# 2. ~/.copaw exists (legacy installation) → use it as-is
# 3. Default → ~/.hanbao
_explicit_working_dir = _get_env("HANBAO_WORKING_DIR")
if _explicit_working_dir:
    ...
    if _legacy_copaw_dir.exists():
        ...
        WORKING_DIR = Path("~/.hanbao").expanduser().resolve()
SECRET_DIR = (
    Path(
...
def _resolve_docs_dir() -> Path | None:
    """Find Hanbao documentation directory across all install methods."""
...
JOBS_FILE = EnvVarLoader.get_str("HANBAO_JOBS_FILE", "jobs.json")
...
CONFIG_FILE = EnvVarLoader.get_str("HANBAO_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("HANBAO_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("HANBAO_OPENAPI_DOCS", False)
...
# Plugin directory (installed via `hanbao plugin install`)
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "HANBAO_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: HANBAO_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("HANBAO_CORS_ORIGINS", "").strip()

# Upload size limit (MB).  None = no limit.
UPLOAD_MAX_SIZE_MB: int | None = (
    int(v)
    if (v := EnvVarLoader.get_str("HANBAO_UPLOAD_MAX_SIZE_MB", ""))
    .strip()
    .isdigit()
    else None
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            _get_env("HANBAO_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "300"),
        ),
        1.0,
    )
except (TypeError, ValueError):
...
try:
    TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL = max(
        float(
            _get_env("HANBAO_TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL", "15"),
        ),
        5.0,
    )
except (TypeError, ValueError):
