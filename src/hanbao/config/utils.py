...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps HANBAO_WORKING_DIR effective even if user config files contain
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    hanbao.channels), filtered by HANBAO_ENABLED_CHANNELS or
    HANBAO_DISABLED_CHANNELS when set.

    * HANBAO_ENABLED_CHANNELS — whitelist (only these channels are active).
    * HANBAO_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, HANBAO_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("HANBAO_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env HANBAO_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_hanbao_running() -> bool:
    """Check if Hanbao is currently running by checking API availability.

    Returns:
        True if Hanbao is running, False otherwise
    """
    try:
