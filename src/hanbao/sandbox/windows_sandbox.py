...
def _save_container_metadata(
    ...
    Args:
        state_dir: Hanbao state directory (``~/.hanbao``).
...
class WindowsSandbox:
    ...
    async def __aenter__(self):
        ...
        if not _is_admin():
            print(
                "[Hanbao Sandbox] WARNING: Not running as administrator. "
                "Sandbox ACL setup may fail.",
            )

        fingerprint = _compute_acl_fingerprint(self._config)
        self._container_name = f"hanbao_{fingerprint[:12]}"
        ...
        if existing_sid:
            ...
            print(
                f"[Hanbao Sandbox] Reusing existing sandbox "
            ...
            print(
                f"[Hanbao Sandbox] Sandbox '{self._container_name}' "
...
def shutdown_cleanup() -> None:
    ...
    For each container metadata file in ~/.hanbao/containers/:
