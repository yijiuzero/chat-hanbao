...
# Sandbox user group
SANDBOX_USERS_GROUP = "HanbaoUsers"
...
def _provision_sandbox_user(username: str, password: str) -> bool:
    ...
    _ensure_local_group(
        SANDBOX_USERS_GROUP,
        "Hanbao sandbox internal group (managed)",
    )
...
def _ensure_python_dir_group_acl() -> None:
    """Grants RX to the HanbaoUsers group on the Python install directory.
    ...
    marker_path = os.path.join(python_dir, ".hanbao_acl_granted")
...
def _remove_python_dir_acl_marker() -> None:
    """Removes the .hanbao_acl_granted marker and resets the flag.

    Targets the Python install directory's marker file.

    Called during shutdown_cleanup so that the next sandbox creation will
    re-grant the ACL to the (re-created) HanbaoUsers group. Without this,
...
_sandbox_state_dir = (
    Path(os.environ.get("USERPROFILE", os.path.expanduser("~"))) / ".hanbao"
)
...
def _create_new_sandbox(
    ...
    sandbox_id = f"hanbao_{fingerprint[:12]}"
    username = sandbox_id
    password = _random_password()
    ...
    # machine-name suffix (e.g. "hanbao_xxx.DESKTOP-E7LJ27U").
...
async def _acquire_sandbox(config: SandboxConfig) -> _SandboxInstance:
    ...
    sandbox_name = f"hanbao_{fingerprint[:12]}"
...
def shutdown_cleanup() -> None:
    ...
    by other still-running Hanbao processes are left untouched.
