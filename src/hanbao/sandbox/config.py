...
This is the natural entry point into the :mod:`hanbao.sandbox` package:
...
Backend layout (``create_sandbox`` dispatches to these by ``SandboxMode``):
  - SEATBELT     → mod:`hanbao.sandbox.macos_sandbox`      (MacOSSandbox)
  - BUBBLEWRAP   → mod:`hanbao.sandbox.bubblewrap_sandbox` (BubblewrapSandbox)
  - LANDLOCK     → mod:`hanbao.sandbox.linux_sandbox`      (LinuxSandbox)
  - APPCONTAINER → Windows sandbox, dispatched on ``allow_read_all``:
      True  → windows_restricted_sandbox (WindowsRestrictedSandbox)
      False → mod:`hanbao.sandbox.windows_sandbox`            (WindowsSandbox)
  - NONE         → mod:`hanbao.sandbox.local_sandbox`      (NoneSandbox)

Shared base class for all backends:
  - :class:`hanbao.sandbox.local_sandbox.LocalSandbox`
