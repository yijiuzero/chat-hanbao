...
# home to Hanbao (I-006). The upload path is disabled (see
...
def _detect_install_method() -> str:
    """Detect how Hanbao was installed based on environment signals."""
    from ..constant import EnvVarLoader

    if EnvVarLoader.get_bool("HANBAO_RUNNING_IN_CONTAINER"):
        return "docker"
    if EnvVarLoader.get_bool("HANBAO_DESKTOP_APP"):
        return "desktop"
    return "pip"
...
def get_system_info() -> dict[str, Any]:
    ...
    - hanbao_version: Hanbao version string
    - install_method: How Hanbao was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running hanbao (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as hanbao_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "hanbao_version": _safe_get(lambda: hanbao_ver, "unknown"),
...
def _upload_telemetry_sync(data: dict[str, Any]) -> bool:
    ...
    hanbao is an independent fork and must NOT phone home to Hanbao's
...
def _get_current_version() -> str:
    """Get the current Hanbao version string."""
    try:
        from ..__version__ import __version__ as hanbao_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when Hanbao is upgraded (or downgraded) to a
    version that hasn't been collected before.

    Args:
        working_dir: Path to Hanbao working directory
    ...
    try:
        ...
        # v1.1 compat: single hanbao_version field
        return marker_data.get("hanbao_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("hanbao_version", "")
                    if old_ver:
