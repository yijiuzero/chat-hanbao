...
def auto_disable_sandbox_on_windows() -> None:
    ...
    Called once during startup (both ``hanbao app`` and the Tauri backend).
    ...
    try:
        ...
        if config.security.sandbox_enabled:
            logger.warning(
                ...
                "privileges are required for the sandbox, but Hanbao is not "
                "running as administrator. The sandbox will be inactive for "
                "this session. To use the sandbox, close Hanbao and relaunch "
