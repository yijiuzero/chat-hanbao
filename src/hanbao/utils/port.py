...
HANBAO_DESKTOP_PORT environment variable.
"""
...
from ..constant import HANBAO_DESKTOP_PORT
...
def get_stable_port(
    ...
    If HANBAO_DESKTOP_PORT environment variable is set, it takes priority
    and the port is fixed (not persisted).
    """
    # Check for user-configured port via environment variable
    if HANBAO_DESKTOP_PORT:
        try:
            port = int(HANBAO_DESKTOP_PORT)
        except (TypeError, ValueError):
            logger.warning(
                "HANBAO_DESKTOP_PORT=%r is not a valid "
        ...
        if port is not None:
            if not 1024 <= port <= 65535:
                logger.warning(
                    "HANBAO_DESKTOP_PORT=%d out of range "
                ...
                if sock:
                    logger.info(
                        "Using HANBAO_DESKTOP_PORT: %d",
                        port,
                    )
                    return port, sock
                logger.warning(
                    "HANBAO_DESKTOP_PORT=%d is "
