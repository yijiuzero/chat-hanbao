...
class HanbaoProvider:
    ...
    async def search(
        ...
        try:
            ...
            raise RuntimeError(
                f"Hanbao search returned HTTP {e.response.status_code}",
            ) from e
        if not isinstance(body, dict) or not body.get("success", True):
            ...
            raise RuntimeError(f"Hanbao search failed: {message}")
