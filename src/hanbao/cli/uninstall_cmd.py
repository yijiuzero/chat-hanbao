# -*- coding: utf-8 -*-
"""hanbao uninstall — remove the Hanbao environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove Hanbao PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# Hanbao" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# Hanbao\nexport PATH=\"\$HOME/\.hanbao/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove Hanbao environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL Hanbao data in {wd}")
    else:
        click.echo(
            "This will remove the Hanbao Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("Hanbao uninstalled. Please restart your terminal.")
