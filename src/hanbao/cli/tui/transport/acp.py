# -*- coding: utf-8 -*-
"""Default TUI transport: drive ``hanbao acp`` over ACP/stdio.

The transport spawns ``hanbao acp`` as a subprocess and connects an ACP
client to it (the same ``spawn_agent_process`` plumbing the in-process ACP
service uses, see ``hanbao/agents/acp/service.py``). All agent capabilities —
...
hooks for the Hanbao agent, so we implement exactly those.
"""
...
# ``_meta`` flag marking the warmup session ephemeral so the Hanbao ACP server
...
def _kill_process_tree(pid: int) -> None:
    ...
    try:
        import psutil  # local import; psutil is already a Hanbao dep
...
class _TuiClient:
    ...
    async def ext_notification(
        ...
        if method in ("hanbao/push_message", "session/push_message"):
...
class AcpTransport:
    """Spawns ``hanbao acp`` and drives one session over ACP/stdio."""
    ...
    def __init__(
        ...
        *,
        ...
        # Default: re-invoke this very interpreter as `python -m hanbao acp`.
    ...
    async def start(self) -> Connected:
        ...
        return Connected(
            ...
            model=model,
            hanbao_version=_agent_version(initialized),
    ...
    async def list_sessions(self) -> list[SessionSummary]:
        if self._conn is None:
            raise RuntimeError("transport not started")
        # No cwd filter: Hanbao runs every session in its single workspace
...
# ``_meta`` key Hanbao sets on the session response with the resolved agent
# id; mirrors the ACP server's ``ACP_AGENT_META_KEY``.
_AGENT_META_KEY = "hanbao.agent"
