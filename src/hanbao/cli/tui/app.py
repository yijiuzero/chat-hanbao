...
class PawApp(App):
    ...
    def __init__(
        ...
        *,
        ...
        # Whether the "hanbao" label has been shown for the current turn.
        self._labeled = False
        ...
        # The model Hanbao reports it is using (read-only, for the status bar
    ...
    async def _ensure_turn_label(self) -> None:
        """Mount the ``hanbao`` label once per turn, above the first piece
    ...
    async def _submit(self, text: str) -> None:
        ...
        # new "hanbao" label) is created for the reply.
    ...
    async def _handle_local_command(self, raw: str) -> None:
        ...
        match command:
            ...
            case _:
                # Everything else (including Hanbao's own slash commands such
    ...
    def _on_connected(self, ev: Connected) -> None:
        ...
        self._status().set(
            ...
            model=ev.model or "—",
            hanbao_version=ev.hanbao_version or "—",
    ...
    def _no_response_text(self, stop_reason: str | None) -> str:
        ...
        return (
            ...
            # [hanbao modification] Brand display name
            "may be misconfigured in hanbao — check it with `hanbao "
            "doctor` or `hanbao models config-key`."
        )
