...
``hanbao``                    open an interactive chat with the active agent
``hanbao tui``                same, with explicit options
``hanbao tui --agent NAME``   chat with a specific agent
``hanbao tui --resume ID``    resume a previous session and continue it

The TUI spawns ``hanbao acp`` using the *current* interpreter
(``python -m hanbao acp --local-diagnostics``), so it always drives the same
install/venv it ships in -- no reliance on ``hanbao`` being on ``PATH``.

Textual and the transport are imported lazily so ``hanbao --help`` and other
...
def _build_transport(
    *,
    ...
    ``[sys.executable, "-m", "hanbao", "acp", "--local-diagnostics"]`` --
    ...
    *not* appended here: ``AcpTransport`` appends ``--agent <id>`` itself when
    ...
    description = (
        f"hanbao acp ({sys.executable} -m hanbao acp --local-diagnostics)"
    )
    if project_dir:
