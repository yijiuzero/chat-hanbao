...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  hanbao agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  hanbao agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      hanbao agents chat --from-agent bot_a --to-agent bot_b --text "..."
      hanbao agents create --name "Research Bot" --agent-id research_bot
      hanbao agents delete research_bot
      hanbao agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      hanbao agents list
      hanbao agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `hanbao agents list`.

    \b
    Examples:
      hanbao agents delete research
      hanbao agents delete research --remove-workspace
      hanbao agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'hanbao agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'hanbao chats list' to find existing sessions (optional)
