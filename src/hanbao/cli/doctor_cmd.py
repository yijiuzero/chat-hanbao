# -*- coding: utf-8 -*-
"""`hanbao doctor` — read-only checks.

`hanbao doctor fix` — conservative repairs with backup.
"""
...
def _check_api_health(
    ...
    try:
        ...
        click.echo(
            f"Hint: start the server with `hanbao app` (default {base}).",
            err=True,
        )
        return False, None
...
def _doctor_server_python_mismatch_note(
    ...
    if server_exe and doctor_exe:
        if not _same_python_executable(doctor_exe, server_exe):
            return (
                "This `hanbao doctor` is not using the same Python "
                "executable as the running `hanbao app` — diagnostics and "
    ...
    if doctor_env.strip() != server_env.strip():
        return (
            ...
            f"`hanbao app` (doctor: {doctor_env!r}; server: "
            f"{server_env!r}). "
...
def _check_web_auth(base: str) -> tuple[bool, str]:
    ...
    if not has_registered_users():
        return (
            ...
            f"        1) Start `hanbao app`, open {base}/ in a browser.\n"
            "        2) Complete registration (single user) on the login "
            "page.\n"
            "        For automation, set HANBAO_AUTH_USERNAME and "
            "HANBAO_AUTH_PASSWORD (legacy COPAW_* names still work) — the "
...
async def _check_active_llm(
    ...
    if (
        ...
        return (
            False,
            "no active LLM slot — run `hanbao models list` and configure "
            "an active model",
            [],
        )
...
def run_doctor_checks(
    ...
    """Run read-only ``hanbao doctor`` checks (no disk mutations)."""
    ...
    if config_ok:
        ...
        _doctor_fix_hint(
            ...
            "`hanbao doctor fix --dry-run --help` and `--only`.",
        )
    ...
    if raw_cfg is not None:
        ...
        if unknown:
            ...
            _doctor_fix_hint(
                ...
                "(`hanbao doctor` and `doctor fix` do not strip unknown keys "
                "yet).",
            )
    ...
    if config_ok:
        ...
        if ws_ok:
            ...
            _doctor_fix_hint(
                "Preview the plan (no writes): `hanbao doctor fix --dry-run "
        ...
        if not ext_nonempty:
            click.echo(
                click.style("OK", fg="green")
                + " — no extension notes (register via hanbao.doctor entry "
        ...
        click.echo(
            ...
            + _skipped_when_cfg_invalid
            + ". Fix the config file, then re-run `hanbao doctor`.",
        )
        click.echo("\n=== Browser (browser_use / Playwright) ===")
    ...
    if wd_ok:
        ...
        _doctor_fix_hint(
            "Fix: set `HANBAO_WORKING_DIR` (or legacy `COPAW_WORKING_DIR`) "
            "or run `hanbao init`. "
            "Preview the plan (no writes): `hanbao doctor fix --dry-run "
    ...
    if llm_ok:
        ...
        _doctor_fix_hint(
            "`hanbao models list` / console model settings — not a "
            "filesystem fix.",
        )
