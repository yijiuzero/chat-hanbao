...
class InstallInfo:
    """Information about the current Hanbao installation."""
...
class RunningServiceInfo:
    """Detected Hanbao service endpoint state."""
...
def _fetch_pypi_release_data() -> dict[str, Any]:
    """Fetch hanbao release metadata from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest Hanbao version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest Hanbao version: {exc}",
        ) from exc
    if not isinstance(data, dict):
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            "latest Hanbao version.",
        )
    return data
...
def _select_latest_version(
    ...
    *,
    ...
    if include_prerelease:
        ...
        if not version:
            raise click.ClickException(
                "Unable to determine the latest Hanbao version.",
            )
        return version
    ...
    if not candidates:
        raise click.ClickException(
            "Unable to determine the latest stable Hanbao version.",
        )
    return str(max(candidates))


def _fetch_latest_version(*, include_prerelease: bool = False) -> str:
    """Fetch the latest published Hanbao version from PyPI."""
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("hanbao")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running Hanbao HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local Hanbao service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_hanbao_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a Hanbao HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running Hanbao service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"Hanbao service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `hanbao shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING HANBAO SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `hanbao shutdown` will forcibly terminate the current "
        "Hanbao backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `hanbao shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `hanbao shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "hanbao"]
    ...
    click.echo("")
    click.echo("Running `hanbao shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `hanbao shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`hanbao shutdown` failed. Please stop the running Hanbao "
            "service manually before running `hanbao update`.",
        )
...
def _build_upgrade_command(
    ...
    *,
    ...
    package_spec = f"hanbao=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from hanbao.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[hanbao] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[hanbao] Updating Hanbao "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[hanbao] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[hanbao] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[hanbao] Update completed successfully.")
        click.echo(
            "[hanbao] Please restart any running Hanbao service "
            "to use the new version.",
        )
    else:
        click.echo(f"[hanbao] Update failed with exit code {return_code}.")
        click.echo(
            "[hanbao] Please fix the error above and run "
            "`hanbao update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool, prerelease: bool) -> None:
    """Upgrade Hanbao in the current Python environment."""
    ...
    if version_check is False:
        click.echo("Hanbao is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `hanbao update`, or rerun "
                "without `--yes` to confirm a forced `hanbao shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `hanbao shutdown`. "
                "Please stop it manually before running `hanbao update`.",
            )

    if not yes and not click.confirm(
        f"Update Hanbao to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
