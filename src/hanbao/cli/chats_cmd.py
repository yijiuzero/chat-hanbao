...
def chats_group() -> None:
    ...
    \b
    Common examples:
      hanbao chats list                    # List all chats
      hanbao chats list --user-id alice    # Filter by user
      hanbao chats get <chat_id>           # View details
      hanbao chats create --session-id s1 --user-id u1
      hanbao chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      hanbao chats list
      hanbao chats list --user-id alice
      hanbao chats list --channel discord
      hanbao chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `hanbao chats list`.
...
def create_chat(
    ...
    Inline creation examples:
      hanbao chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      hanbao chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      hanbao chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      hanbao chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
