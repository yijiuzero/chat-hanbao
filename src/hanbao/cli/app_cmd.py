...
def _warn_if_auth_off_non_loopback_bind(host: str, port: int) -> None:
    """Warn when Hanbao is reachable beyond loopback without auth."""
    ...
    warning = f"""
============================================================
SECURITY NOTICE: Hanbao is bound to {bind_address} without authentication.

Anyone who can reach this address may access Hanbao APIs without login.
...
def app_cmd(
    ...
    if workers is not None:
        ...
        click.echo(
            "   Hanbao always uses 1 worker for stability. "
    ...
    if reload:
        os.environ["HANBAO_RELOAD_MODE"] = "1"
    else:
        os.environ.pop("HANBAO_RELOAD_MODE", None)
