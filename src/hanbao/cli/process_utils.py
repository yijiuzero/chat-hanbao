...
def _matches_hanbao_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a Hanbao CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m hanbao {subcommand}",
            f" hanbao {subcommand}",
            f"__main__.py {subcommand}",
            f'hanbao.exe" {subcommand}',
            f"hanbao.exe {subcommand}",
        )
    )


def _is_hanbao_service_command(command: str) -> bool:
    """Return whether the command line looks like a local Hanbao app."""
    return _matches_hanbao_cli_command(command, "app")


def _is_hanbao_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a Hanbao CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "hanbao" or _matches_hanbao_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local Hanbao service."""
    if not host:
        return []
