# -*- coding: utf-8 -*-
"""Read-only diagnostics for `hanbao doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``hanbao.app._app`` lifespan).
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `hanbao app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``hanbao app``), when ``GET /api/doctor/runtime`` returned
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``hanbao doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``hanbao app`` may run an automatic migration;
    ...
    return (
        ...
        "`hanbao app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `hanbao app` once (or see docs / `hanbao init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("HANBAO_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "HANBAO_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- Hanbao checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~hanbao.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `hanbao doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    from ..utils.console_static import (
        CONSOLE_STATIC_ENV,
        find_hanbao_source_repo_root,
        resolve_console_static_dir,
    )

    notes: list[str] = []
    env_dir = EnvVarLoader.get_str("HANBAO_CONSOLE_STATIC_DIR", "").strip()
    if env_dir:
        notes.append(
