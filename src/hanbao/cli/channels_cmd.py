...
# [hanbao modification] removed discord/telegram/voice/yuanbao/slack names
_ALL_CHANNEL_NAMES = {
    "imessage": "iMessage",
    "dingtalk": "DingTalk",
    "feishu": "Feishu",
...
def configure_wechat(current_config: WeChatConfig) -> WeChatConfig:
    ...
    bot_token_file = click.prompt(
        "bot_token file path",
        default=(
            current_config.bot_token_file or "~/.hanbao/wechat_bot_token"
        ),
        type=str,
    )
...
# [hanbao modification] removed discord/telegram/voice configurators
...
@click.option(
    "--target-user",
    required=True,
    help=("Target user ID (REQUIRED, get from 'hanbao chats list' query)"),
)
@click.option(
    "--target-session",
    required=True,
    help=("Target session ID (REQUIRED, get from 'hanbao chats list' query)"),
)
@click.option(
    "--text",
    required=True,
...
def send_cmd(
    ...
    Complete Usage Flow:
      Step 1 - Query available sessions (REQUIRED):
        hanbao chats list --agent-id my_bot --channel console
      ...
      Step 3 - Send message using queried parameters:
        hanbao channels send --agent-id my_bot --channel console \\
    ...
    Examples with jq automation:
      ...
      SESSIONS=$(hanbao chats list --agent-id bot --channel console)
      USER=$(echo "$SESSIONS" | jq -r '.[0].user_id')
      SESSION=$(echo "$SESSIONS" | jq -r '.[0].session_id')

      # Send message
      hanbao channels send --agent-id bot --channel console \\
        --target-user "$USER" --target-session "$SESSION" \\
        --text "Automated notification"

    \b
    Prerequisites:
      1. MUST use 'hanbao chats list' to get valid target-user and
         target-session
