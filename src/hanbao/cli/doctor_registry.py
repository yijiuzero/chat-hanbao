# -*- coding: utf-8 -*-
"""Extensible hooks for `hanbao doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``hanbao.doctor``::
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("hanbao.doctor", "copaw.doctor"):
